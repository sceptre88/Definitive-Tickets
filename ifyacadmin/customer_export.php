...
<?php
  ...
  $customers_query_raw = "select c.customers_id,
	...
	$customers_info_query=tep_db_query('select customers_info_date_account_created as account_created from '.TABLE_CUSTOMERS_INFO." where customers_info_id='" . $customers_id . "'");						
	$customers_info_array=tep_db_fetch_array($customers_info_query);
	$created=substr($customers_info_array['account_created'],0,10);	
...
?>
                 ...
                 <tr class="dataTableRow">
                	  <td class="dataTableContent"><b><?php echo $customers_id; ?></b></td>
                      ...
                      <td class="dataTableContent"><?php echo $created; ?></td>
                    </tr>
<?php
    }
?>
...
else
{
	...
	$sep= str_replace(',', ",", $sep);
	$contents="customers_id".$sep."customers_lastname".$sep."customers_firstname".$sep."customers_email_address".$sep."customers_gender".$sep."entry_company".$sep."entry_street_address".$sep."entry_city".$sep."entry_suburb".$sep."entry_postcode".$sep."entry_state".$sep."countries_name".$sep."customers_telephone".$sep."customers_fax\n";
	$customers_query_raw = "select c.customers_id,
    							  c.customers_lastname,
    							  c.customers_firstname,
    							  c.customers_email_address,
	...
	$customers_info_query=tep_db_query('select customers_info_date_account_last_modified as last_modified, customers_info_date_account_created as account_created from '.TABLE_CUSTOMERS_INFO." where customers_info_id='" . $customers_id . "'");						
	$customers_info_array=tep_db_fetch_array($customers_info_query);
	$created=$customers_info_array['account_created'];
	$street_address=$row['entry_street_address'];
	$postcode=$row['entry_postcode'];
	$city=$row['entry_city'];
	$state=$row['entry_state'];
	$suburb=$row['entry_suburb'];
	$country=$row['countries_name'];
		...
		$contents.=str_replace(",", "",$street_address) . ',' . str_replace(",", "",$city) . ',' . str_replace(",", "",$suburb) . ',' . $postcode . ',' . $state .',' . $country.$sep;
        $contents.=$row['customers_telephone'].$sep;
		$contents.=substr($customers_info_array['account_created'],0,10)."\n";
	}
