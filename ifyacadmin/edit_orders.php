...
define('ENABLE_SSL', '');
define('HINT_PRESS_VOUCHER_UPDATE', '');
 ...
 if($_POST['actval'] == 'updateordsta')
 {
     ...
     if($ordid != '')
     {
		//2023 this code updates(over wirtes) the most recent entry
        //it also updates ALL ticked orders without waiting to see if emails were went		- moved down ~ 20 lines
...
{
	//2023 - individual update code
						date_default_timezone_set(STORE_TIMEZONE);
						$today = date("Y-m-d H:i:s"); 
	
					tep_db_query("update orders set orders_status = 3 where orders_id = '" . $oID . "'");
					    //update status history
					$sql_data_array = array('orders_id' => $oID,
					'orders_status_id' => 3,
					'date_added' => $today, 
					'customer_notified' => '1',
					'comments' => 'Order email sent');

					tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
					
					tep_db_query("update orders_products set orders_products_status = 3 where orders_id = '" . $oID . "'");
...
########################## email finish #################
 ...
 //
  ...
  if (isset($action)) 
  {
    ...
    // Update Order
      case 'update_order':
	   ...
	   ### season tickets           
		...
		$order_id = tep_db_prepare_input($_GET['oID']);
        $status = tep_db_prepare_input($_POST['status']);
	   ### season tickets 
	  try {
	    if(function_exists('update_season_queue')){
			update_season_queue((int)$order_id, $status );			
			}
		} catch (Exception $e) {
			unset($e);
		}
		...
		'billing_phone' => tep_db_input(tep_db_prepare_input(((isset($_POST['billing_same_as_customer']) && $_POST['billing_same_as_customer'] == 'on') ? $_POST['update_billing_phone'] : $_POST['update_billing_phone']))),
	...
	'delivery_phone' => tep_db_input(tep_db_prepare_input(((isset($_POST['shipping_same_as_billing']) && $_POST['shipping_same_as_billing'] == 'on') ? (($_POST['billing_same_as_customer'] == 'on') ? $_POST['update_delivery_phone'] : $_POST['update_billing_phone']) : $_POST['update_delivery_phone']))),
    ...
    'last_modified' => getServerDate(false),
	);
	...
	{
	//2023 set marker if change is to 3 (delivered)
	  $message_flag = 0;
	  if($_POST['status']== 3){$message_flag = 3;}
	//end
        tep_db_query("UPDATE " . TABLE_ORDERS . " SET 
...
// to deal with that
	// if (CR_ACTIVE=="True" && file_exists("includes/admin_cr_dbadd.php") && $_POST['status']=="3"){
		...
		//print_r($_POST);die;
		 // Notify Customer ?
			$customer_notified = '0';
		//2023 add || $message_flag == "3" option so that email gets sent for all changes to 'delivered'
			if (isset($_POST['notify']) && ($_POST['notify'] == 'on') || $message_flag == 3) 
			{
                            //echo "III";
			  $notify_comments = '';
			...
			concat(LTRIM(c.customers_lastname),' ',LTRIM(c.customers_firstname)) as customers_name, 
			o.payment_method, o.date_purchased, o.billing_name, o.billing_phone, o.delivery_phone, o.last_modified, o.currency, o.currency_value, s.orders_status_name, op.products_type, ot.text as order_total from " . TABLE_ORDERS . " o left join " . TABLE_ORDERS_TOTAL . " ot on (o.orders_id = ot.orders_id), " . TABLE_ORDERS_STATUS . " s, " .TABLE_ORDERS_PRODUCTS." op, ".TABLE_CUSTOMERS ." c 
			where  
			o.customers_id=c.customers_id 
			and o.orders_status = s.orders_status_id 
			and o.orders_id=op.orders_id 
			and s.language_id = '" . (int)$customers_language . "' 
			and ot.class = 'ot_total' 
			and o.orders_id='".$oID ."' 
			order by o.orders_id DESC,customers_name ASC");
...
//die;
		...
		$product_comments = '';
          foreach($_POST['update_products'] as $orders_products_id => $products_details) {
			...
			$product_comments .= " " . $products_details['qty']. " x " . $products_details['model'] . " ". $products_details['name'] . " deleted from order ";
			    ...
			    if (STOCK_LIMITED == 'true')
				{
						...
						$family_ticket=$products_details["qty"];
							...
							tep_db_query("UPDATE " . TABLE_PRODUCTS . " SET products_status='1',products_title_1='',
							products_quantity = products_quantity + " . $family_ticket . ",
                    ...
                    tep_db_query("UPDATE " . TABLE_ORDERS_PRODUCTS . "  
					              SET 
								  products_model = '',
								  products_name = '".TEXT_DELETED_RESTOCKED . oe_html_quotes($products_details["name"]) . "',
								  products_price = '0',
								  is_printable = '0',
								  final_price = '0',
								  products_tax = '',
					...
					tep_db_query("DELETE FROM orders_tickets 
	                              WHERE orders_id = '" . (int)$oID . "'
                                  AND orders_products_id = '" . (int)$orders_products_id . "'");

					tep_db_query("DELETE FROM orders_barcode
	                              WHERE orders_id = '" . (int)$oID . "'
                                  AND products_id = '" . (int)$orders_products_id . "'");
					...
					#### barcode and tickets
					  $barcode_query = tep_db_query("select created, barcode from orders_barcode where products_id = '".(int)$orders_products_id."' and orders_id = '" . $oID . "'");
  					  $barcode = tep_db_fetch_array($barcode_query); 	
  						if(tep_not_null($barcode['created'])){

	 						//delete files
							 
							//delete barcode table entry
		   ...
		   elseif ( (isset($products_details['voucher'])) && ($products_details['voucher'] == 'on') )
		   {
		     $new_price = 0;
			 $new_price = $products_details["qty"]*$products_details["price_incl"];
				...
				do {
					$unique_coupon_code = ucfirst(substr($_SESSION['login_last_name'],0,1)).
					                      ucfirst(substr($_SESSION['login_first_name'],0,1)).
										  tep_create_random_value(4,'digits').
										 // '-'.$_POST['update_custId'];
										 '-'.$oID;
					$q = tep_db_query("SELECT * FROM coupons WHERE coupon_code =' ".$unique_coupon_code."'");
				 } while(mysqli_num_rows($q) > 0);

			$startdate=strtotime("today");
			$startingdate=date(EVENTS_DATE_FORMAT,strtotime("today"));
			$enddate =  date(EVENTS_DATE_FORMAT,strtotime("+ 12 months",$startdate));
			
			$sql_data_array = array('coupon_code' => $unique_coupon_code,
									'coupon_amount' =>$products_details["total_incl"],
									'coupon_type' => "F",
									'coupon_active' => "Y",
									'uses_per_coupon' => 1,
									'coupon_tax_class_id' => "",
									'uses_per_user' => 1,
									'uses_per_order' => 1,
									'coupon_minimum_order' => 1,
									'orders_id' => (int)$oID,	
									'restrict_to_customers' => $_POST['update_custId'],						
									'coupon_start_date' => tep_convert_date_raw($startingdate),
									'coupon_expire_date' => tep_convert_date_raw($enddate),
									'date_created' => getServerDate(),
									'date_modified' => getServerDate());
			
			define('DEFAULT_VOUCHER_ID','');
			define('DEFAULT_VOUCHER_ISSUED','');
			define('DEFAULT_VOUCHER_CATEGORY','');
			define('DEFAULT_VOUCHER_PID','');
		
			$languages = tep_get_languages();
//fix enddate GORDON
				$cpn_desc = sprintf(TEXT_COUPON_VALIDITY, $enddate) . sprintf(TEXT_COUPON_ORDER_REFERS,$oID);
	
					$sql_data_marray = array('coupon_name' => tep_db_prepare_input($unique_coupon_code),
		   ...
		   $product_comments .= " " . $products_details['qty']. " x " . $products_details['name'] . " [" . $products_details['model'] . ']' . DEFAULT_VOUCHER_ISSUED .$unique_coupon_code . ". ";

			tep_db_query("UPDATE " . TABLE_ORDERS_PRODUCTS . "  
						  SET 
						  events_id = '".$products_details["qty"]."',
						  products_model = '".DEFAULT_VOUCHER_ID."',
						  products_name = '". DEFAULT_VOUCHER_ISSUED .' '.$unique_coupon_code. "',
						  products_quantity = '1',
						 products_tax = '0',
						  products_sku = '6',
						  events_type = 'C',
						  support_packs_type = 'C',
						  categories_name = '".DEFAULT_VOUCHER_CATEGORY."',
						  concert_date = '',
						  concert_time = '',
						  is_printable = '1'
						  WHERE orders_id = '" . (int)$oID . "'
...
tep_db_query("UPDATE " . TABLE_ORDERS_PRODUCTS . " SET products_price='".$new_price."',final_price='".$new_price."' WHERE orders_id = '" . (int)$oID . "'
								  AND orders_products_id = '" . (int)$orders_products_id . "'");
  ...
  elseif (isset($_POST['nC2'])) {
	   tep_redirect(tep_href_link(FILENAME_ORDERS_EDIT, tep_get_all_get_params(array('action')) . 'action=email2'));
  }
...
//------insert customer choosen option eof ----
		...
		if ($order->products[$i]['products_type']=='P')
		{
			...
			$products_ordered .= '<tr class="textTableContent" height="56">' . 
									...
									// hide SKU '<td valign="top" class="textMain"><small><br>' . $order->products[$i]['sku'] . '</small></td>' .
...
//==============================================
	...
	$merge_details[BILL_PH]=$order->billing['customer_phone'];
	//$merge_details[BILL_BE]=$order->billing['customer_email'];
	if($order->billing['customer_email']==''){
	...
	$merge_details[DELI_PH]=$order->delivery['customer_phone'];
	$merge_details[DELI_DE]=$order->delivery['email'];
	...
	$merge_details[TEXT_FB]=EMAIL_TEXT_DATE_ORDERED . ' ' . utf8_encode(strftime('%A %B %d, %Y', strtotime($order->info['date_purchased'])));
	//$merge_details[TEXT_FB]=utf8_encode(strftime('%A %B %d, %Y', strtotime($order->info['date_purchased'])));
...
//	$merge_details['Detailed_PDF_Format_Link']='<a href="' . tep_catalog_href_link(FILENAME_PDF_MAIL, 'file=pdf&order_id=' . $insert_id, 'SSL', false) . '" style="font-family:Arial, Helvetica, sans-serif;font-size:13px;color:#3966B1;font-weight:bold;text-decoration:none;">' . TEST_MAIL_PDF . '</a>';
	...
	if($check_billing['customers_country']=="Box Office"){
		...
		try
		  {
		      require_once('includes/functions/voucher_ticket.php');
			  $filename= (DIR_FS_CATALOG.create_checkout_pdf($insert_id));
			  }
 ...
 ?>
	...
	?>
          ...
          <ul>
			...
			<li><?php echo '<a href="' . tep_href_link('voucher_ticket.php', 'oID=' . $_GET['oID']) . '" TARGET="_blank">' . tep_image_button('button_ticket.gif', 'VOUCHERS') . '</a>'; ?></li>
...
<!- eof 5.0.8 -->							
	...
	<input type="hidden" id="update_custId" name="update_custId" value="<?php echo stripslashes($order->customer['id']); ?>">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border: 1px solid #C9C9C9;">
              ...
              <tr id="shippingAddressEntry">
                ...
                <table width="100%" cellspacing="0" cellpadding="2">
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right"><?php echo ENTRY_PHONE; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_delivery_phone" size="37" value="<?php echo stripslashes($order->delivery['customer_phone']); ?>" <?php if ('ORDER_EDITOR_USE_AJAX' == 'true') { ?>onChange="updateOrdersField('delivery_phone', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
				  <tr class="dataTableRow"> 
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" style="border: 1px solid #C9C9C9;">
              ...
              <tr id="billingAddressEntry">
                ...
                <table width="100%" cellspacing="0" cellpadding="2">               
				  ...
				  <tr class="dataTableRow"> 
                    <td class="dataTableContent" valign="middle" align="right" nowrap><?php echo ENTRY_PHONE; ?></td>
                    <td colspan="3" valign="top" class="dataTableContent"><input name="update_billing_phone" size="37" value="<?php echo stripslashes($order->billing['customer_phone']); ?>" <?php if ('ORDER_EDITOR_USE_AJAX' == 'true') { ?>onChange="updateOrdersField('billing_phone', encodeURIComponent(this.value))"<?php } ?>></td>
                  </tr>
				  <tr class="dataTableRow"> 
    ...
    for ($i=0; $i<sizeof($order->products); $i++) {
      $orders_products_id = $order->products[$i]['orders_products_id'];  ?>
			   ...
			   <tr class="dataTableRow">
				...
				<td class="dataTableContent" valign="top">
					<div align="center">
						<span class="" onMouseover="ddrivetip('<?php echo oe_html_no_quote(HINT_DEL_VOUCH); ?>')"; onMouseout="hideddrivetip()"><script language="JavaScript" type="text/javascript">
                   <!--
                    document.write("<img src=\"images/icon_info.gif\" border= \"0\" width=\"13\" height=\"13\">");
	               //-->
                  </script></span><input type="checkbox" name="<?php echo "update_products[" . $orders_products_id . "][delete]"; ?>">
						 
				   	<?php echo TABLE_TEXT_OR;?>
						<input type="checkbox" name="<?php echo "update_products[" . $orders_products_id . "][voucher]"; ?>">
						</div>
				</td>
				...
				<?php //sakwoya - we need to make the quantity box a non field here for products type=P
					  ...
					  // means less files to change in the standard osConcert setup
							...
							if(($product_type=='P')or($product_type=='C'))
							{
								?>
...
?>                </td>
    ...
    <!-- Begin Update Block, only for non-javascript browsers -->
	 ...
	 <div class="updateBlock2">
                <div class="update1"><?php echo HINT_PRESS_VOUCHER_UPDATE; ?></div>
              <div class="update2">&nbsp;</div>
              <div class="update3">&nbsp;</div>
              <div class="update4" align="center"><?php echo UPDATE_VOUCHERS; ?>&nbsp;<?php echo tep_draw_checkbox_field('nC2', '', false); ?></div>
              <div class="update5" align="center"><?php echo tep_image_submit('button_update.gif', IMAGE_UPDATE); ?></div>
          </div>
	...
	<table style="border: 1px solid #C9C9C9;margin-top:30px;" cellspacing="0" cellpadding="2" class="dataTableRow" id="commentsTable">
     <tr class="dataTableHeadingRow">
      ...
      $orders_history_query = tep_db_query("SELECT orders_status_history_id, orders_status_id, date_added, customer_notified, comments, field_1, field_2, field_3, field_4, other 
	   ...
	   if ($orders_history['customer_notified'] == '1') {
        ...
        echo '    <td class="dataTableHeadingContent" align="left" width="10">&nbsp;</td>' . "\n" .
			 ...
			 //Check if ticket was ever printed osconcert
