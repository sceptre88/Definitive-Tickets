...
<?php
   $orders_query_raw = "select o.*,o.customers_email_address,o.orders_id, o.customers_name, o.payment_method, o.date_purchased, o.last_modified, o.currency, o.currency_value, s.orders_status_name, ot.value as order_total
	...
	if(isset($_POST['status']) && $_POST['status'] !="")
{
 ...
 if(tep_db_num_rows($orders_query) && isset($_POST['SalesSubmit']))
 	...
 	?>
 	 <tr>
          ...
          <tr class="dataTableHeadingTitleRow">
			...
			<td class="dataTableHeadingTitleContent" align="right"><?php echo TABLE_HEADING_CUST_ID; ?></td>
			...
			<td class="dataTableHeadingTitleContent" align="right"><?php echo TABLE_HEADING_PAYMENT; ?></td>
 	...
 	<?php
 	fputs($fp,TABLE_HEADING_CUST_ID .";");
	...
	fputs($fp,TABLE_HEADING_PAYMENT .";");
	...
	fputs($fp,TABLE_HEADING_TAX .";");
 	...
 	while ($orders = tep_db_fetch_array($orders_query)) 
 	{
			...
			$sqlsub = "select value,class from orders_total where orders_id=".$orders['orders_id']." and class='ot_subtotal'"; 
			$sql_subtotal=tep_db_query($sqlsub);
			$gross1=tep_db_fetch_array($sql_subtotal);
			//$gross=$gross1['value'];
			$gross = number_format($gross1['value'], 2);
			...
			$sqlsub = "select value,class from orders_total where orders_id=".$orders['orders_id']." and class='ot_service_fee'"; 
			$sql_servicefee=tep_db_query($sqlsub);
			$servicefee1=tep_db_fetch_array($sql_servicefee);
			//$servicefee=$servicefee1['value'];
			$servicefee = number_format($servicefee1['value'], 2);
			...
			$sqlsub = "select text,class from orders_total where orders_id=".$orders['orders_id']." and class='ot_advance_fee'"; 
			$sql_advancefee=tep_db_query($sqlsub);
			$advancefee1=tep_db_fetch_array($sql_advancefee);
			$gv=$advancefee1['text'];
			
			$sqlsub = "select value,class from orders_total where orders_id=".$orders['orders_id']." and class='ot_coupon'"; 
			$sql_coupon=tep_db_query($sqlsub);
			$coupon1=tep_db_fetch_array($sql_coupon);
			//$coupon=$coupon1['value'];
			$coupon = number_format($coupon1['value'], 2);
			...
			$sqlsub = "select value,class from orders_total where orders_id=".$orders['orders_id']." and class='ot_tax'"; 
    ...
    <tr class="dataTableRow">
		<td class="dataTableContent"><?php echo '<a href="' . tep_href_link(FILENAME_ORDERS, tep_get_all_get_params(array('oID', 'action')) . 'oID=' . $orders['orders_id'] . '&action=edit') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $orders['customers_id']; ?></td>
		...
		<td class="dataTableContent" align="right"><?php echo strip_tags(number_format($orders['order_total'], 2)); ?></td>
		</tr>
		   
    <?php
fputs($fp,strip_tags($orders['customers_id']).";");
