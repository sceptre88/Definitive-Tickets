...
define('TEXT_MASTER','- Master Quantity');
...
$seats_query = tep_db_query("select 
		distinct
		p.products_id, 
		sum(case when p.products_status = '0' and p.product_type = 'P'  and p.products_sku = '1' then 1 else 0 end) as all_sold_P,
	...
	{	
		...
		sum(case when op.orders_products_status = '1' then (op.products_quantity) else 0 end) as all_pending,
		sum(case when o.customers_country = 'Box Office' and o.orders_status = 3 then (op.products_quantity) else 0 end) as box_office_seats,
		...
		where
			p.products_id = '" . $seats['products_id'] . "' AND p.products_model = op.products_model
	    group by
			p.products_id"); 
		...
		while($all_sold_result = tep_db_fetch_array($all_sold_query)){	
		 ...
		 $all_pending = $all_pending + (int)$all_sold_result['all_pending'];
...
if ($category['categories_quantity'] > 0) { //GA with limited admissions
...
echo '</td>';
echo '<td>';
if($category['categories_GA']=='1'){
echo "<strong>" . $category['categories_name'] . "</strong> [" . $category['concert_venue'] . "]." . "<strong> " .$category['categories_quantity'] . " " . TEXT_MASTER . "</strong><br>";
echo $ga_remaining . " " . TEXT_FREE_SEATS . " " . $all_box_office . " ". TEXT_BOX_OFFICE_TICKETS ." " . $online . " ". TEXT_SOLD_ONLINE . " " . $all_invitati . " " . TEXT_INVITATIONS . " " . TEXT_PENDING . " (" . $all_pending . "), " . $all_sold . " " . TEXT_SOLD_TOTAL . "<br>";
}else{
echo "<strong>" . $category['categories_name'] . "</strong> [" . $category['concert_venue'] . "] [" . $category['date_id'] . "]<br>";
echo $all_unsold . " " . TEXT_FREE_SEATS . " " . $all_box_office . " ". TEXT_BOX_OFFICE_TICKETS ." " . $all_sold_P . " ". TEXT_SOLD_ONLINE . " " . $all_invitati . " " . TEXT_INVITATIONS . " " . $all_sold . " " . TEXT_SOLD_TOTAL . "<br>";
}
echo '</td>';
echo '</tr>';
echo '</table>';
