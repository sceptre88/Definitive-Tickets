...
// Sets the status of a product
  function tep_set_product_status($products_id, $status) {
    if ($status == '1') 
	{
      return tep_db_query("update " . TABLE_PRODUCTS . " set products_status = '1', products_ordered = '0', products_sku = '1',products_quantity = '1',  products_last_modified = now() where products_id = '" . (int)$products_id . "'");
    } 
	elseif ($status == '0') 
	{
...
// Retreive server information
  function tep_get_system_information($link = 'db_link') {
    global $FREQUEST;
	global $$link;
...
//kodes
    return array('date' => tep_datetime_short(date('Y-m-d H:i:s')),
                 ...
                 'db_version' => 'MySQL ' . (function_exists('mysqli_get_server_info') ? mysqli_get_server_info($$link) : ''),
...
// Add tax to a products price
  function tep_add_tax($price, $tax) {
	...
	{
      //PHP8 ?
	  return tep_round(floatval($price), $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']) + tep_calculate_tax($price, $tax);
    } else {
      return tep_round(floatval($price), $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']);
    }
  }
...
// Calculates Tax rounding the result
  function tep_calculate_tax($price, $tax) {
    global $currencies;
    return tep_round(floatval($price) * floatval($tax) / 100, $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']);
  }
                  ...
                  */
                  ...
                  /*
									...
									*/
									...
									//ORDR_DD=>TEST_ORDR_DD, 
                  //ORDR_PD=>TEST_ORDR_PD,
									...
									//DELI_CA=>TEST_ORDR_CA, 
                  BILL_NA=>TEST_ORDR_CF,
									//BILL_PH=>TEST_ORDR_PH,
									...
									BILL_CU=>TEST_ORDR_CU, 
                  //BILL_CO=>TEST_ORDR_CO,
 ...
 #######cartzone we are using the support_packs_type as a guide to distinguish the difference between a GA order and a MULTI seat reservation########
  function tep_remove_order_events($order_id, $restock = false) {
    if ($restock == 'on') {
	  ...
	  {
	  	 //osconcert restock FAMILY tickets
...
// If insufficent stock is available return an out of stock message
 ...
 function tep_cfg_pull_down_prod_view($config_value,$key=''){
  ...
  function tep_get_products_array_single($first=''){
		...
		//osconcert dumped some query to get refunds to work basically
		$products_sql = "select pd.products_name,pd.products_id from  " . TABLE_PRODUCTS . " p," . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " pcd " . 
...
function clear_ticket_tables($input, $order = 0){
  ...
  function tep_process_stock_global_notification_emails($product = array(), $products_id, $language_directory, $language_id, $audience = array(), $currencies) {
    ...
    if (defined('ENABLE_LOW_STOCK_ALERT') && ENABLE_LOW_STOCK_ALERT == 'true') {
      ...
      $products_query = tep_db_query("select products_id, products_quantity from products where products_quantity <= " . STOCK_REORDER_LEVEL . " order by products_id desc");
...
//echo "select sale_specials_condition, sale_deduction_value, sale_deduction_type, sale_id from " . TABLE_SALEMAKER_SALES . " where " . $option_where . " ((sale_categories_all='' and sale_products_selected='') or sale_categories_all like '%," . $category . ",%' or sale_products_selected like '%," . (int)$product_id .",%') and sale_status = '1' and (sale_date_start <= '" . $sDate. "' or sale_date_start = '0000-00-00') and (sale_date_end >= '" . $sDate . "' or sale_date_end = '0000-00-00') and (sale_pricerange_from <= '" . $product_price . "' or sale_pricerange_from = '0') and (sale_pricerange_to >= '" . $product_price . "' or sale_pricerange_to = '0')";
 ...
 function tep_convert_datetime_raw($date,$format='d-m-Y',$sep_char="/-/"){
   ...
   function get_customer_group_id(){
    ...
    // config function for EXPIRY INTERVALS
  function tep_cfg_time_expire($value,$key='') {
    $format=array(array('id'=>'+1800','text'=>'Expire 30 minutes AFTER the show start date & time'),
	              array('id'=>'+3600','text'=>'Expire one hour AFTER the show start date & time'),
				  array('id'=>'+7200','text'=>'Expire two hours AFTER the show start date & time'),
				  array('id'=>'+9000','text'=>'Expire three hours AFTER the show start date & time'),
				  array('id'=>'0','text'=>'No Expiry Difference'),
				  array('id'=>'-1800','text'=>'Expire 30 minutes BEFORE the show start date & time'),
				  array('id'=>'-3600','text'=>'Expire 60 minutes BEFORE the show start date & time'),
				  array('id'=>'-5400','text'=>'Expire 90 minutes BEFORE the show start date & time'),
				  array('id'=>'-7200','text'=>'Expire 2 hours BEFORE the show start date & time'),
				  array('id'=>'-9000','text'=>'Expire 3 hours BEFORE the show start date & time'));
    $name = (($key) ? 'configuration[' . $key . ']' : 'configuration_value');
	$result=tep_draw_pull_down_menu($name,$format,$value) . '&nbsp;';
	return $result;
  }
