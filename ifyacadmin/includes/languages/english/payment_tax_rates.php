<?php
/*
  Copyright (c) 2021 osConcert
*/
 // Check to ensure this file is included in osConcert!
defined('_FEXEC') or die(); 

define('HEADING_TITLE', 'Tax Rates');

define('HEADING_NEW_TITLE', 'New tax rates');
define('TABLE_HEADING_TAX_RATE_PRIORITY', 'Priority');
define('TABLE_HEADING_TAX_CLASS_TITLE', 'Tax Class');
define('TABLE_HEADING_COUNTRIES_NAME', 'Country');
define('TABLE_HEADING_ZONE', 'Zone');
define('TABLE_HEADING_TAX_RATE', 'Tax Rate');
define('TABLE_HEADING_ACTION', 'Action');
define('HEADING_TITLE_SEARCH', 'Search');
define('TEXT_INFO_EDIT_INTRO', 'Please make any necessary changes');
define('TEXT_INFO_DATE_ADDED', 'Date Added:');
define('TEXT_INFO_LAST_MODIFIED', 'Last Modified:');
define('TEXT_INFO_CLASS_TITLE', 'Tax Class Title:');
define('TEXT_INFO_COUNTRY_NAME', 'Country:');
define('TEXT_INFO_ZONE_NAME', 'Zone:');
define('TEXT_INFO_TAX_RATE', 'Tax Rate (%):');
define('TEXT_INFO_TAX_RATE_PRIORITY', 'Tax rates at the same priority are added, others are compounded.<br><br>Priority:');
define('TEXT_INFO_RATE_DESCRIPTION', 'Description:');
define('TEXT_INFO_INSERT_INTRO', 'Please enter the new tax class with its related data');
define('TEXT_INFO_DELETE_INTRO', 'Are you sure you want to delete this tax rate?');
define('TEXT_INFO_HEADING_NEW_TAX_RATE', 'New Tax Rate');
define('TEXT_INFO_HEADING_EDIT_TAX_RATE', 'Edit Tax Rate');
define('TEXT_INFO_HEADING_DELETE_TAX_RATE', 'Delete Tax Rate');
define('TEXT_EMPTY_GROUPS', 'No records found');


define('TEXT_LOADING_DATA', 'Loading data....');
define('UPDATE_DATA','Daten aktualisieren');
define('TEXT_UPDATE_ORDER','Updating...');
define('INFO_SEARCHING_DATA', 'Searching data....');
define('TEXT_PRD_DELETING','Deleting...');
define('TEXT_INFO_HEADING_DELETE_GROUP', 'Delete Group');

define('IMAGE_DELETE_TAX_RATE','Delete Tax Rate');
define('TEXT_DELETE_INTRO','Are you sure to delete this Tax Rate?');
define('TEXT_TAX_RATE_DELETE_SUCCESS','Tax Rate Deleted Successfully...');
define('TEXT_TAX_RATE_INSERT_SUCCESS','Tax Rate inserted successfully...');
define('TEXT_TAX_RATE_NOT_DELETED','Tax rate Not Deleted');
define('ERROR_IN_DATABASE_TRY_AGAIN','<font color=red>we cannot able to sort this tax rate.</font>');
define('ERROR_TAX_RATES_TITLE', 'Please enter the tax rate title.');
define('ERROR_TAX_RATES_DESCRIPTION','Please enter the description value.');
define('ERROR_NUMERIC_VALUE','Rate Value should contain Numeric Values Only.');
define('TEXT_EMPTY_TAX_RATES','No Tax Rates Found');
define('TEXT_RECORDS','Records');
?>
