...
class salesMaker
	...
	function doEdit()
	{
		...
		$row=0; 
		//PHP8
		//if (empty($sInfo->sale_discount_type)){
...
<?php
	...
	function doUpdate()
	{
	 ...
	 //PHP8 problem sizeof here is wrong 
	 //Fatal error: Uncaught TypeError: sizeof(): Argument #1 ($value) must be of type Countable|array, null given
	 if(!is_array($unsel_list)){
		$unsel_list=array();
	 }
	 if(!is_array($sel_prd_list)){
		$sel_prd_list=array();
			 }
			//if(sizeof($categories)>0 || sizeof($unsel_list)>0 || sizeof($sel_prd_list)>0) {
				if ((is_countable($categories) && sizeof($categories) > 0) || 
				(is_countable($unsel_list) && sizeof($unsel_list) > 0) || 
				(is_countable($sel_prd_list) && sizeof($sel_prd_list) > 0)) {
...
//Fatal error: Uncaught TypeError: substr(): Argument #1 ($string) must be of type string, array given in C:\xampp\htdocs\2023\stefan\osconcert\admin\includes\tweak\display\salesMaker.php:508 Stack trace: #0 C:\xampp\htdocs\2023\stefan\osconcert\admin\includes\tweak\display\salesMaker.php(508): substr(Array, 1) #1 C:\xampp\htdocs\2023\stefan\osconcert\admin\includes\tweak\general.php(90): salesMaker->doUpdate() #2 C:\xampp\htdocs\2023\stefan\osconcert\admin\sales_maker.php(18): checkAJAX('SALESMAKER') #3 {main} thrown in C:\xampp\htdocs\2023\stefan\osconcert\admin\includes\tweak\display\salesMaker.php on line 508
		if($select_pro_ids!='')
