...
/*
	...
	IMPORTANT!
	line 365 we take it for granted 'refunded' orders status=5
*/
...
class customersOrders{
    ...
    function doCustomersOrdersList($search='',$filter_param='',$order)
    {
		...
		//PHP8 take away fp
		//and ot.class = 'ot_total' ".$search . " ".$filter_param." group by o.orders_id, 
        ...
        if($order == 0)
        {
           $orders_query_raw = "select
			  ...
			  from orders
			  o left join orders_total ot on (o.orders_id = ot.orders_id),
			  orders_status s,
			  orders_products op
       ...
       $orders_query=tep_db_query($orders_query_raw);
        ...
        if($found)
        {
            ...
            while($order_result=tep_db_fetch_array($orders_query))
            {
				...
				$show_comments='';
				$show=$order_result["customers_dummy_account"];
				if($show==1){
					$show_comments='&nbsp;&nbsp;'.tep_image(DIR_WS_IMAGES.'icons/plus.gif');
				}
		...
		//if($search!="" || ($filter_param!='' && !$found))
			if (!empty($search) || (!empty($filter_param) && !$found))
        { ?>
...
if($chk_advise=='Y')
{
			...
			$order_result=tep_db_fetch_array($orders_query_raw);			
				...
				if($chk_type=='F')//full refunds restock
				{
					...
					//REFUNDED is alway 5??
					tep_db_query("update " . TABLE_ORDERS . " set orders_status='5' where orders_id ='" .(int)$oID . "'");
						...
						for($jcnt=0;$jcnt<sizeof($parray);$jcnt++){
							...
							if($FREQUEST->postvalue('chk_restock' . $parray[$jcnt])=='Y'){
								...
								$events_result=tep_db_fetch_array($events_query);
									...
									//REFUNDED is alway 5??
									tep_db_query("update " . TABLE_ORDERS_PRODUCTS . " set products_quantity=0, orders_products_status='5' where orders_products_id ='" . (int)$parray[$jcnt] . "'");
...
//                                                'text' => $orders_status['orders_status_name']);
                            ...
                            <tr><td class="main"><b><?php echo ENTRY_STATUS; ?></b><td width="200"><?php
								...
								<?php //osconcert altered 'notify customer' checkbox to OFF here (false)
...
// if (empty($filter)) {
	// $filter_param=" and o.orders_status>0 ";
// }else{
	// $filter_param=" and o.orders_status='".$filter."'";
// }
// correct fix PHP8
$filter = $FREQUEST->getvalue('filter');
$filter_param = empty($filter) ? ' and o.orders_status>0 ' : " and o.orders_status='$filter'";
...
if ($search!=''){
	$filter_search=$FREQUEST->getvalue('filter_search');
	  //Graeme - we could use a switch statement here but maybe just go with if, else else
		...
		elseif($filter_search==9){
			$search_param = "and op.coupon_amount like '%" . tep_db_input($search) . "%'";
		}
		else{
			$search_param='';
...
<!--	<table border="0" width="100%" id="<?php echo $this->type;?>Table">
	...
	$orders_history_query = tep_db_query("select comments,customer_notified from " . TABLE_ORDERS_STATUS_HISTORY . " where orders_id = '" . (int)$order_id . "' LIMIT 1");
	if (tep_db_num_rows($orders_history_query)) {
		while ($orders_history = tep_db_fetch_array($orders_history_query)) {
			
			if($orders_history['customer_notified']==0){
	...
	$rep_array=array(	"ORD_DATE_ADDED"=>((format_date($order_result["date_purchased"])=='')?(""):(TEXT_DATE_ORDER_CREATED .  format_date($order_result["date_purchased"]))),
			...
			"ORD_COMMENTS"=>TEXT_INFO_PAYMENT_COMMENTS.$comments2.' <br><br><span class="Y">'.$comments.'</span>',
			"UPDATE_RESULT"=>'doDisplayResult'
	);
	echo mergeTemplate($rep_array,$template);
	$jsData->VARS["updateMenu"]=",normal,";
        ...
        <!--Added column Purchaser in case of edit of customer name-->
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                    ...
                    <td width="2%" id="##TYPE####ID##"><input title="Quick Order Update" type="checkbox" name="ord_status[]" class="ordchkcls" value="##ID##"></td>
                    ...
                    <td width="10%" align="left" class="main" style="padding-left:20px" onclick="javascript:doDisplayAction({'id':##ID##,'get':'##ROW_CLICK_GET##','result':doDisplayResult,'style':'boxRow','type':'##TYPE##','params':'oID=##ID##'});" id="##TYPE####ID##status"><span class="">##STATUS_NAME##</span>##SHOW_COMMENTS##</td>
    ...
    <tr>
        ...
        <td>
            <table border="0" cellpadding="5" cellspacing="0" width="100%">
				...
				<tr>
                    <td colspan="3" class="main">##ORD_COMMENTS##</td>
