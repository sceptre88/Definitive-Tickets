...
class salesCoupon
{
	...
	function doCheckCode() {
		...
		if($code!="" && $coupon_id>0) {
			...
			if(tep_db_num_rows($coupon_code_check_query)>0) 
				echo 'Err. Coupon Code already Exists';
		 }
	}
	...
	function doList($status){
		...
		if ($this->pagination){
			...
			$query_split->maxRows=1000;//MAX_DISPLAY_SEARCH_RESULTS;
...
<?php
		...
		if ($tax_rate>0) $gross_amount=$amount+($amount*$tax_rate/100);
        if (substr($FREQUEST->postvalue('coupon_amount'), -1) == '%'){
            $gross_amount =number_format(floatval($gross_amount), 2, '.', '').'%';
        }
?>
		<tr>
			  <td width="2%">&nbsp;</td>
...
<?php
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0" height="100%">
					<tr class="dataTableHeadingRow">
						...
						<table border="0" cellpadding="0" cellspacing="0" width="100%">
							<tr>
								<td class="main" width="20%"><b><?php echo COUPON_NAME;?></b></td>
								<td class="main" width="20%"><b><?php echo COUPON_AMOUNT;?></b></td>
	...
	function doEdit() {
		...
		if ($tax_rate>0) $gross_amount=$amount+($amount*$tax_rate/100);
			...
			if ($coupon['coupon_type']=='P') {
			  $coupon_amount .= '%';
              $amount=number_format(floatval($coupon_amount), 2, '.', '').'%';
            //  $gross_amount =number_format($gross_amount, 2, '.', '').'%';
              $gross_amount =number_format(floatval($coupon_amount), 2, '.', '').'%';
			}
...
?>
				...
				<td align="left" class="main" width="30%"><?php echo COUPON_STARTDATE; ?></td>
				<td align="left" width="30%"><?php echo tep_draw_date_selector('coupon_startdate', mktime(0,0,0, $coupon_startdate[1],(int)$coupon_startdate[2], $coupon_startdate[0])); ?></td>
				<td align="left" class="main"><?php echo COUPON_STARTDATE_HELP; ?></td>
       </tr>
       <tr>
				<td align="left" class="main" width="30%"><?php echo COUPON_FINISHDATE; ?></td>
				<td align="left" width="30%"><?php echo tep_draw_date_selector('coupon_finishdate', mktime(0,0,0, $coupon_finishdate[1],(int) $coupon_finishdate[2], $coupon_finishdate[0])); ?></td>
...
?>
  ...
  <a onClick="javascript:return doUpdateAction({
      'id': <?php echo $coupon_id; ?>,
      'get': 'PreviewEmail',
      'imgUpdate': true,
      'type': 'salCoupon',
      'style': 'boxRow',
      ...
      'uptForm': 'mail',
      'extraFunc': textEditorRemove,
      'customUpdate': doMailUpdate('PreviewMail'),
      'result': doDisplayResult,
      'params': 'rID=<?php echo $coupon_id; ?>',
      'message1': page.template['UPDATE_DATA'],
      'message': page.template['UPDATE_IMAGE']
  });">
    <?php echo tep_image_button('button_send_email.gif', IMAGE_SEND_EMAIL); ?>
  </a>
</td>
...
<?php
 ...
 function display_categories($type,$prev_ids=''){
	...
	if (is_countable($prev_ids) && count($prev_ids)>0){
	$prev_len = $prev_ids;
	}
	switch ($type){
		case 'p': 
