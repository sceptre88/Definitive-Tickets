        ...
        if($search!='')
{
	...
	$where.=' ) and ';
        ...
        //		print_r($FSESSION);
        $categories_sql = "select c.categories_products_lock, c.categories_status, c.categories_GA, c.categories_shipping, c.categories_quantity_remaining, c.categories_advance_fee,c.categories_id, cd.concert_venue, c.date_id, cd.concert_date, cd.concert_time, c.categories_is_printable, c.bg_height, c.categories_status, cd.categories_name, c.plan_id, c.parent_id, c.section_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where " . $lock . " ".$where ." c.categories_id = cd.categories_id and cd.language_id = '" . (int)$FSESSION->languages_id . "'  and c.parent_id = '" . (int)$parent_id . "' order by c.sort_order";
        ...
        while($categories_result=tep_db_fetch_array($categories_query)){
			...
			// $concert_date='';
			// }else{			
			 $concert_date=$categories_result["concert_date"]. ' ' .$categories_result["concert_time"];
			// }

            $rep_array=array(       "PAD_LEFT"=>$level*10,
                                    "CAT_ID"=>$categories_result["categories_id"],
                                    "CAT_PARENT"=>$parent_id,
									"CAT_SECTION_ID"=>$categories_result["section_id"],
									...
									"CAT_FEE"=>$categories_result["categories_advance_fee"],
    ...
    function doCategoryInfo($category_id)
	{
        ...
        $category_query=tep_db_query("select c.categories_id, c.parent_id,c.manufacturers_id, date_format(c.date_added,'%Y-%m-%d') as date_added, date_format(c.last_modified,'%Y-%m-%d') as last_modified,cd.categories_id,c.categories_image, c.categories_image_2,c.categories_image_3,c.categories_image_4, c.plan_id,c.categories_quantity, c.categories_GA,  c.categories_is_printable, c.categories_shipping, c.bg_height,c.section_id,cd.section_id,c.categories_quantity_remaining, c.categories_advance_fee,cd.categories_description,cd.categories_name from ".TABLE_CATEGORIES."  c, ".TABLE_CATEGORIES_DESCRIPTION." cd where c.categories_id=cd.categories_id and cd.language_id='".(int)$FSESSION->languages_id."' and c.categories_id='".(int)$category_id."' limit 1");
        ...
        if (tep_db_num_rows($category_query)>0)
		{
            ...
            $rep_array=array(	    "CAT_DATE_ADDED"=>$date_added,
									...
									"CAT_SECTION_ID"=>$category_result["section_id"],
									"CAT_FEE"=>$category_result["categories_advance_fee"],
    ...
    function doCategoryUpdate()
	{
		...
		$categories_advance_fee =$FREQUEST->postvalue('categories_advance_fee');
        ...
        if ($category_id>0)
		{
			...
			if ($categories_plan_id == '9') {
				$update_array["plan_id"] = 9;
			} elseif ($categories_plan_id > 4 && $categories_plan_id < 9) {
			...
			//check if there has been a value posted for shipping - if so update the products
			$shipping_weight='0';
			tep_db_query("UPDATE products p, products_to_categories p2c SET p.products_weight= '".$shipping_weight."' WHERE p.products_id=p2c.products_id and p.product_type = 'P'");
			$update_array["categories_shipping"]=0;
			}
			
			$update_array["categories_advance_fee"]=$categories_advance_fee;
		...
		else 
		{
			...
			$update_array["plan_id"]=$categories_plan_id;//set the plan id when creating category
			$update_array["bg_height"]='';//=$categories_bg_height;
		...
		$categories_advance_fee=$FREQUEST->getRefValue('categories_advance_fee','POST');
        ...
        for ($icnt=0,$n=count($LANGUAGES);$icnt<$n;$icnt++)
		{
            ...
            } else 
			{
				...
				$update_array["section_id"]=(int)$insert_id;
				$update_array["venue_id"]='';
				$new_section_id = tep_db_insert_id();
				
				tep_db_perform(TABLE_CATEGORIES_DESCRIPTION,$update_array);
				
				tep_db_query( "UPDATE categories SET section_id = '". $new_section_id."' where categories_id= '".$new_section_id."'" );
	...
	// {
        // global $FREQUEST,$jsData;
        // $category_id=$FREQUEST->getvalue('cID','int',0);
        // tep_remove_category($category_id);
        // $jsData->VARS["deleteRow"]=array("id"=>$category_id,"type"=>"cat");
        // $jsData->VARS["reduceTree"]=$category_id;
    ...
    function doCategoryDelete() 
	{
        ...
        foreach ( $subcategories_array as $key => $value ) {
			...
			//ADDED MAY 2022 TO TRULY DELETE PRODUCTS
			// 2023 - section_id is not cloning correctly in pd table - fixed
			tep_db_query("delete from " . TABLE_PRODUCTS . " where parent_id = '" . (int)$key . "'");
			tep_db_query("delete from " . TABLE_PRODUCTS . " where section_id = '" . (int)$key . "'");// 2023 remove the parent_id == 0 
			tep_db_query("delete from " . TABLE_PRODUCTS_DESCRIPTION . " where section_id = '" . (int)$key . "'"); //
			tep_db_query("delete from " . TABLE_PRODUCTS_TO_CATEGORIES . " where section_id = '" . (int)$key . "'");
    ...
    function doCategoryMove()
	{
		...
		tep_db_query("update " . TABLE_CATEGORIES . " set section_id = " . $new_parent_id . ", last_modified = now() where section_id = " . $category_id);
		tep_db_query("update " . TABLE_CATEGORIES_DESCRIPTION . " set section_id = " . $new_parent_id . " where section_id = " . $category_id);
    ...
    global $FREQUEST,$jsData, $subcategories_array, $old_and_new_cat_array;
    ...
    $new_parent_id_array = $subcategories_array;
    //$new_parent_id_array[ 0 ] = 0; //2023
	//exit (var_dump($new_parent_id_array));
// $key is the old_cat_id $value is its old parent_id
// 2023 n.b. new_cat here gets passed to clone_all_prods where it is used as the parent_id
    foreach ( $subcategories_array as $key => $value ) {
        $new_cat = $this->tep_insert_cloned_category( $key, 1, $value);
       // $this->tep_clone_all_prods( $key, $new_cat, $_SESSION["new_section_id"] ); 
	   //$this->tep_clone_all_prods_2023( $key, $new_cat, $_SESSION["new_section_id"] ); 
    }
	$this->tep_clone_all_prods_2023( $key, $new_cat, $_SESSION["old_section_id"] ); 
     $this->doCategories();   
    }
...
// using p2c table
// 2023 this misses cat_id == 0 so use section_id to find those??
// 2023 $new_cat gets used as parent_id
#################################################################

function tep_clone_all_prods_2023( $old_cat, $new_cat, $old_section_id ) { 
	global $new_parent_id_array;
	global $old_and_new_cat_array;
    $products_query_result = tep_db_query( "SELECT * from products_to_categories where section_id = '" . $old_section_id . "'" );

    while ( $product = tep_db_fetch_array( $products_query_result ) ) {
		$old_cat = $product['categories_id'];
		$new_cat = $new_parent_id_array [$old_cat];
		$new_section_id = $_SESSION["new_section_id"] ;
        $this->tep_clone_products( $product[ 'products_id' ], $new_cat, $new_section_id );
    }
//2023 try this to get the empty products over
	$products_query_result = tep_db_query( "SELECT * from products_to_categories where categories_id = '0' and section_id = '" . $old_cat . "'" );

    while ( $product = tep_db_fetch_array( $products_query_result ) ) {
      //  $this->tep_clone_products( $product[ 'products_id' ],0, $new_section_id); 
    }
}	
#################################################################
// gets all products in one category and clones them into another
...
function tep_clone_all_prods( $old_cat, $new_cat, $new_section_id ) { 
...
// 2023 need to get the first category as the section_id....
// using Session
#################################################################
...
function tep_insert_cloned_category( $cat_to_clone_id, $status = 1, $parent = 0 ) {
    ...
    //tep_db_query( "UPDATE temporary_table SET categories_status = '" . $status . "'" );//2023 remove this line for the present
    tep_db_query( "UPDATE temporary_table SET parent_id = '" . $new_parent_id_array[ ( int )$parent ] . "'" );
	...
	//2023 try to set the section_id in the categories table here
	if ($parent == 0){ 

		$sql = 'SELECT section_id FROM '.TABLE_CATEGORIES.' WHERE categories_id = '.( int )$cat_to_clone_id;
		$old_query = tep_db_fetch_array(tep_db_query($sql));
		$old_section_id = $old_query['section_id'];



		$_SESSION["new_section_id"]	= $new_category_id; 
		$_SESSION["old_section_id"]	= $old_section_id; 

	} 
	tep_db_query( "UPDATE categories SET section_id = '". $_SESSION["new_section_id"]."' where categories_id= '".$new_category_id."'" );
	//2023
    $new_parent_id_array[ $cat_to_clone_id ] = $new_category_id;
	$old_and_new_cat_array [ $cat_to_clone_id ] = $new_category_id;
	...
	tep_db_query( "UPDATE temporary_table SET section_id='" . $_SESSION["new_section_id"] . "'" );
...
function tep_clone_products( $prod_id, $new_cat_id, $new_section_id) { 
    ...
    tep_db_query( "UPDATE temporary_table SET parent_id = '" . $new_section_id . "'" );//**** 2023 todo*/
    tep_db_query( "UPDATE temporary_table SET section_id = '" . $new_section_id . "'" ); 
    tep_db_query( "UPDATE temporary_table SET products_id=NULL" );
	tep_db_query( "UPDATE temporary_table SET products_ordered=0" );
	...
	tep_db_query( "UPDATE temporary_table SET section_id = '" . $new_section_id . "'" );//added 2023
    tep_db_query( "INSERT INTO products_description SELECT * FROM temporary_table" );
    tep_db_query( "DROP TABLE temporary_table" );

    // add an entry into P2C

    tep_db_query( "INSERT INTO `products_to_categories` (`products_id`, `categories_id`, `section_id`) VALUES ('" . ( int )$new_product_id . "', '" . ( int )$new_cat_id . "', '" . ( int )$new_section_id . "');" );
   //tep_db_query( "INSERT INTO `products_to_categories` (`products_id`, `old_products_id`, `categories_id`, `section_id`) VALUES ('" . ( int )$new_product_id . "', '" . ( int )$prod_id . "','" . ( int )$new_cat_id . "', '" . ( int )$new_section_id . "');" );
    ...
    function doProductsList($where='',$category_id=0,$search='')
	{
        ...
        $query_split=false;
        $products_sql='select pd.products_id,pd.products_name,p.products_quantity,p.product_type,p.products_date_available,p.products_price,p.color_code,p.products_status,p.products_ordered,p.section_id,p.parent_id,ptc.categories_id from '.TABLE_PRODUCTS.' p, '.TABLE_PRODUCTS_DESCRIPTION." pd, ".TABLE_PRODUCTS_TO_CATEGORIES." ptc where p.products_id=pd.products_id and ptc.products_id=pd.products_id and pd.language_id='" . $FSESSION->languages_id . "' $where $orderBy";
	...
	if($products_result["product_type"]=='Q' || $products_result["product_type"]=='L')
		...
		$warning="";
		//This product status may not be correct. Button reset maybe required. (showing ordered but status available)
		if($products_result["product_type"]=='P' && $products_result["products_ordered"]>0 && $products_result["products_status"]==1){
			$warning="blue";
		}elseif($products_result["product_type"]=='P' && $products_result["products_quantity"]<1 && $products_result["products_status"]==1){
			$warning="red";
		}else{
			$warning="";
		}
    ...
    function doProductChangeStatus()
	{
		...
		$blocked=$FREQUEST->getvalue("blocked","int",0);
        $category_id=$FREQUEST->getvalue("cID","int",0);
        ...
        tep_db_query("UPDATE " . TABLE_PRODUCTS . " set products_status=" . $status . ", products_quantity=" . $status . ",products_ordered=0,author_name='Y',products_last_modified = now() where products_id=$product_id");
	...
	function doProductChangeRowStatus()
	{
		...
		{
        if ($status!=0 && $status!=1) $status=0;//basically if not 1  or 0 its got  to  be  0
        tep_db_query("UPDATE " . TABLE_PRODUCTS . " set products_status=" . $status . ", products_quantity=" . $status . ",products_ordered=0 where products_id=$product_id");
		}else //if its  not a reserved seat
		{
		if ($status!=0 && $status!=1) $status=0;//
        tep_db_query("UPDATE " . TABLE_PRODUCTS . " set products_status=" . $status . ",products_ordered=0 where products_id=$product_id");	
		}
        if ($status==1){
            $result='<a href="javascript:void(0)" onclick="javascript:doSimpleAction({id:'. $product_id .",get:'ProductChangeStatus',result:doSimpleResult,params:'pID=". $product_id . "&status=0',message:'".TEXT_UPDATING_STATUS."'});\">" . tep_image(DIR_WS_IMAGES . 'template/icon_active.gif') . '</a>';
        } else 
    ...
    function doProductInfo($product_id=0)
	{
		...
		}else
		{
			...
			"PRD_EDIT"=>'Stock Status Edit? ' .$product_result["author_name"],
			"PRD_ORD"=>'Ordered? ' .$product_result["products_ordered"],
			"PRD_BLOCKED"=>'Blocked here? ' .$product_result["author_name"].' '.$product_result["products_last_modified"],
			"PRD_DESCRIPTION"=>tep_db_input($product_result["products_description"]),
			"PRD_ID"=>$product_result["products_id"],
			"SECTION_ID"=>$product_result["section_id"],
       ...
       // $sql_array["manufacturers_id"]=$FREQUEST->postvalue('manufacturers_id','int',0);
        ...
        switch($product_type)
		{
            ...
            default:
				...
				/////////////////////////////////////////////////////////////////////
				//cannot change these values for integrated seat plans (P)
				//section_id is usually added as a guide SP id.
				
				//p.parent_id is very important when calling cPath
				//some products are out of this category for a reason so don't change the parent_id
				if (($ID<0) or  ($sql_array["products_w"]>0))
				{
				$sql_array["section_id"]=$FREQUEST->postvalue("category_id","int",0);
				$sql_array["parent_id"]=$FREQUEST->postvalue("category_id","int",0);
				}
           ...
           // $sql_array["products_sku"]=$FREQUEST->postvalue("products_sku");
            ...
            //$sql_array["is_attributes"]=$FREQUEST->postvalue('is_attributes','string','N');//No Salemaker Flag 2024
            ...
            for ($icnt=0,$n=count($LANGUAGES);$icnt<$n;$icnt++)
			{
				...
				{
                    $sql_array["section_id"]=$category_id;
            ...
            } else 
			{
				...
				{
                    //updated
					if($products_status==8 || $sql_array["products_sku"]==0 || $sql_array["product_type"]=='Q' || $sql_array["product_type"]=='L')
					{
						$result=tep_image(DIR_WS_IMAGES . 'template/icon_hidden.gif');
					}else{
...
function doCategoryEdit()
{
	...
	if ($category_id>0)
	{
		$category_query = tep_db_query("select c.manufacturers_id, c.parent_id,c.categories_id, cd.categories_name,cd.categories_heading_title,c.plan_id,c.categories_is_printable, c.categories_shipping,cd.categories_description,c.color_code,c.bg_height, c.categories_image,c.categories_image_2,c.categories_image_3,c.categories_image_4,concert_venue,concert_date,concert_time, c.categories_GA, c.categories_quantity, c.categories_quantity_remaining,c.categories_advance_fee, language_id from  ".TABLE_CATEGORIES."  c,  ".TABLE_CATEGORIES_DESCRIPTION."  cd where c.categories_id = cd.categories_id and c.categories_id='".(int)$category_id."' order by cd.language_id");
		$catInfo=array();
		$langInfo=array();
		while($cat_result=tep_db_fetch_array($category_query))
		{
			...
			"categories_advance_fee"=>tep_db_prepare_input($cat_result["categories_advance_fee"]),
    ...
    <tr>
        <td width="50%" valign="top">
            <table border="0" cellpadding="2" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <td class="main" valign="top"><?php  echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$icnt]['directory'] . '/images/' . $languages[$icnt]['image'], $languages[$icnt]['name']) . '&nbsp;' . tep_draw_input_field('categories_name[' . $languages[$icnt]['id'] . ']', isset($langInfo[$languages[$icnt]['id']]["name"])?stripslashes($langInfo[$languages[$icnt]['id']]["name"]):'', 'size="40"','maxlength="128"'); ?></td>
                </tr>
                <?php 
				} 
				?>
            ...
            <table border="0" cellpadding="2" cellspacing="0" width="100%" bgcolor="">
                ...
                <tr>
                    ...
                    <td class="main" valign="top"><?php  echo tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$icnt]['directory'] . '/images/' . $languages[$icnt]['image'], $languages[$icnt]['name']) . '&nbsp;' . tep_draw_input_field('concert_venue[' . $languages[$icnt]['id'] . ']', isset($langInfo[$languages[$icnt]['id']]["venue"])?stripslashes($langInfo[$languages[$icnt]['id']]["venue"]):'', 'size="40"','maxlength="128"'); ?></td>
                </tr>
                <?php 
				} 
				?>
        ...
        <td valign="top">
            <table border="0" cellspacing="3" cellpadding="3" width="100%">
				...
				<?php  if ($catInfo["parent_id"]==0){ ?>
                <tr>
                    <td class="main" valign="top" width="150">
					<?php echo TEXT_EDIT_CATEGORIES_ADVANCE_FEE; ?></td>
                    <td class="main" valign="top">
					<?php 
					echo tep_draw_input_field('categories_advance_fee',$catInfo["categories_advance_fee"],' size="10"'); ?>
					</td>
                </tr>
                <?php } ?>
		...
		<?php //end row colour?>
			   ...
			   // echo tep_draw_radio_field('products_status', '1', $in_status) . '&nbsp;' . TEXT_PRODUCT_AVAILABLE . '&nbsp;' . tep_draw_radio_field('products_status', '0', $out_status) . '&nbsp;' . TEXT_PRODUCT_NOT_AVAILABLE;
				...
				?>
				<tr>
				<td class="main"  colspan="2" valign="top"><br><?php echo TEXT_DESIGN_MODE_NO; ?></td>
				</tr>
				<?php
				}elseif (DESIGN_MODE=='yes' && $category_id<1)
				{
				?>
				<tr>
				<td class="main"  colspan="2" valign="top"><br><?php echo TEXT_DESIGN_MODE_YES; ?></td>
				</tr>
				<?php
}
				?>
				...
				{ 
					...
					$the_plan_id[]=array('id'=>'1','text'=>SEATPLAN_SUB.'(1)');
					$the_plan_id[]=array('id'=>'9','text'=>GA_MODE.'(9)');
			...
			<?php 
		if (DESIGN_MODE=='yes' && $category_id>0)
		{
			?>
			<table border="0" cellpadding="2" cellspacing="0" width="100%">
			<tr>
		...
		<?php 
				...
				echo tep_draw_input_field('categories_bg_height',$catInfo["bg_height"],' value="' . $catInfo["bg_height"] . '" size="10"');
		...
		<tr>
		  <td width="50%" valign="top">
		   ...
		   <tr <?php echo $the_style;?> id="hidden_2">
            ...
            <div style="width:100%;height:250px;overflow:auto;border: 0px solid black">
                <table border="0" cellspacing="0" cellpadding="3">
				<tr><td colspan="2">
				Make a BUTTON: <pre>&lt;a href="index.php?cPath=978" class="btn btn-primary"&gt;Merchandise&lt;/a&gt;</pre>
				</td><td>BREAK: &lt;br&gt;</td></tr>
...
<?php
 ...
 $delete_message='';
//has plan id like seat plan integrated??
$has_plan_id_query= tep_db_query("select plan_id from " . TABLE_CATEGORIES . " where categories_id= ".$category_id);
$has_plan_id = tep_db_fetch_array($has_plan_id_query); $has_plan_id['plan_id'];

if ($has_plan_id['plan_id']<5){
	...
	//no warning its not a seat plan
	$category_childs = tep_childs_in_category_count($category_id);
    $category_products =tep_products_in_category_count($category_id);
...
$category_id=$FREQUEST->getvalue('cID','int',0);
if (empty($ROW_TREE)) $ROW_TREE=tep_get_row_status_tree('0', '',$category_id);
...
<form action="javascript:void(0)" method="post" enctype="application/x-www-form-urlencoded" name="catCloneSubmit" id="catCloneSubmit" onsubmit="alert('you submitted the form');">
    ...
    <table width="100%"  border="0" cellspacing="5" cellpadding="5" style="padding-left:20px;">
        ...
        <tr>
            <td class="main"><b><?php echo sprintf(TEXT_CLONE_CATEGORIES_INTRO, $category_name);?></b>
			<br>
			<span class="red"><?php 
			echo TEXT_CLONE_CATEGORIES_WARNING;?></span>
			</td>
        </tr>
...
} else 
{
	...
	if((DESIGN_MODE=='no')or($has_plan_id['plan_id']<5))
	{
    $delete_message='<p><span class="smallText">' . TEXT_DELETE_PRODUCT_INTRO . '</span>';
	}else{
	$delete_message='<p><span class="red">' . TEXT_DELETE_PRODUCT_DESIGN . '</span>';	
	}
}
?>
...
################################################################
}elseif ($product->products_status=="8")
{
#############################################################	
	switch ($product->products_status) 
		{
		case '8': $h_status = false; $b_status = true; 
		break;
		case '1':
		default: $h_status = true; $b_status = false;
		}
################################################################
}
                    ...
                    <td valign="top" class="mright">
						...
						{ ?>
                            <tr>
								...
								echo ($default_panel==$panels[$icnt]?"productPanelSelect":"productPanel");?>" onClick="javascript:showPanelContent({id:'<?php echo $panels[$icnt];?>',className:'productPanel','type':'productPanel','extraFunc':<?php echo $panels[$icnt]=="DESCRIPTION"?'doProductEditor':'false';?>});" id="productPanel<?php echo $panels[$icnt];?>menu">
                                    <?php
                                    echo "<a href='javascript:void(0);'><div>" . constant("HEADING_ITEM_" .$panels[$icnt]) . '</div></a>';
									
									if(!isset($jsData->VARS["storePage"]["productPanelMenus"])){
									$jsData->VARS["storePage"]["productPanelMenus"] = array();
									}
...
if ($product->product_type=="L" || $product->product_type=="Q")
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    <td class="main">
                                        <?php
                                        echo '<div class="title">' . TEXT_PRODUCTS_STATUS . '</div>';
                                        echo tep_draw_radio_field('products_status', '1', $h_status) . '&nbsp;' . TEXT_PRODUCT_AVAILABLE . '&nbsp;' . tep_draw_radio_field('products_status', '8', $b_status) . '&nbsp;' . TEXT_PRODUCT_HIDDEN . '';
                                        ?>
                                    </td>
                                </tr>
								<?php
                                    ...
                                    <td class="main" >
                                       ...
                                       <div  style="padding-left:10px">
                                            ...
                                            for ($icnt=0, $n=count($LANGUAGES); $icnt<$n; $icnt++) 
											{
                                                ...
                                                echo '&nbsp;' . tep_draw_input_field('products_url[' . $LANGUAGES[$icnt]['id'] . ']', tep_db_prepare_input($products_url[$LANGUAGES[$icnt]['id']]),'style="width:300px;"  maxlength="128"') . '<br>';
                                            }
                                            ?>
                                        </div>
                                    </td>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                <tr>
                                    <td class="main">
										...
										if(!isset($jsData->VARS["storePage"]["ptypePanel"])){
										$jsData->VARS["storePage"]["ptypePanel"] = array();
										}
				...
				echo $product->product_type=="B" || $product->product_type=="G" || $product->product_type=="X" || $product->product_type=="F" || $product->product_type=="P" || $product->product_type=="L"?'':$display_mode_html;?>>
                                <td class="main">
                                  <?php //  color code
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                <tr>
                                    <td class="main">
										...
										if(!isset($jsData->VARS["storePage"]["pmodePanel"])){
										$jsData->VARS["storePage"]["pmodePanel"] = array();
										}
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tr>
        <td valign="top" width="##CAT_IMAGE_WIDTH##">
		<div style="width:100%;height:100px;overflow:hidden">##CAT_IMAGE##</div><br></td>
        <td width="10">&nbsp;</td>
        <td valign="top" width="40%">
            <table class="main" border="0" cellpadding="1" cellspacing="0" width="100%">
                ...
                ##CAT_DATE_MODIFIED##
				<td class="main">
				<strong>Category ID: ##CAT_ID##</strong><br>
				Design ID:##CAT_MAN_ID##<br>
				Plan ID:##CAT_PLAN_ID##</td>
				<td class="main"></td>
				##CAT_CATEGORIES_QUANTITY##
				##CAT_CATEGORIES_QUANTITY_REMAINING##
				<tr>
                    <td class="main">Section ID: ##CAT_SECTION_ID##</td>
                </tr>
            </table>
        </td>
        <td width="50%" valign="top" class="smallText" align="justify">
        ...
        <td>
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                    ...
                    <td class="main" onClick="javascript:doDisplayAction({'id':##PRD_ID##,'get':'##ROW_CLICK_GET##','result':doDisplayResult,'style':'boxRow','type':'prd','params':'pID=##PRD_ID##&cID=##CAT_ID##'});" id="prd##PRD_ID##title"><span class="##WARNING##">##PRD_NAME## </span><span title="##PRD_PRICE##" class="cube colorband" style="background-color:##CLR_BAND##"></span> </td>
    ...
    <tr>
        ...
        <td valign="top" width="350">
            <table border="0" cellpadding="3" cellspacing="0" width="100%">
				...
				<tr>
                    <td class="main">Section ID: ##SECTION_ID##</td>
                </tr>
				<tr>
				...
				<tr>
                    <td class="main">##PRD_EDIT##</td>
                </tr>
				<tr>
                    <td class="main">##PRD_ORD##</td>
                </tr>
				<tr>
                    <td class="main">##PRD_BLOCKED## (<small>This can show the last time this product was manually deactivated here (with button) AFTER a RESET</small>)</td>
                </tr>
            </table>
        </td>
