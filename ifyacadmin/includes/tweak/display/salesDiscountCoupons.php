...
defined('_FEXEC') or die();
	class salesDiscountCoupons
		...
		function doList(){
			...
			$query_split=false;
			$gv_query_raw = "select c.coupon_amount, c.coupon_type,c.coupon_code,c.coupon_active, c.coupon_id, c.orders_id,cd.coupon_name,cd.coupon_description,c.date_created from " . TABLE_COUPONS . " c, " . TABLE_COUPONS_DESCRIPTION . " cd where c.coupon_id = cd.coupon_id and cd.language_id='" . (int)$FSESSION->languages_id . "'" . $where;
			if ($this->pagination){
			...
			if($found)
			{
				...
				while($gv_result=tep_db_fetch_array($gv_query))
				{
					...
					$rep_array=array(	"ID"=>$gv_result["coupon_id"],
										...
										"DESCRIPTION"=>$gv_result["coupon_description"],
...
?>
							...
							<tr class="dataTableHeadingRow">
								<td valign="top">
									<table border="0" cellpadding="0" cellspacing="0" width="100%">
										...
										<td class="main" width="15%">
										<b><?php echo  TABLE_HEADING_COUPON_DESCRIPTION;?></b>
										</td>
										<td class="main" width="15%">
    ...
    $customer_query = tep_db_query("
        SELECT o.customers_name, o.date_purchased, o.orders_id, ot.value, ot_coupon.value AS coupon_value
        FROM " . TABLE_ORDERS . " o
        JOIN " . TABLE_ORDERS_TOTAL . " ot ON ot.orders_id = o.orders_id AND ot.class = 'ot_total'
        LEFT JOIN " . TABLE_ORDERS_TOTAL . " ot_coupon ON ot_coupon.orders_id = o.orders_id AND ot_coupon.class = 'ot_coupon'
        JOIN " . TABLE_COUPON_REDEEM_TRACK . " ct ON ct.order_id = o.orders_id AND ct.order_id != 0 AND ct.coupon_id = '" . tep_db_input($cid) . "'
    ...
    <div style="padding:5px;height:150px;overflow:auto;">
        <table cellpadding="0" cellspacing="1" bgcolor="#000000;" width="90%" align="center">
            <tr class="opencontent">
                <td width="15%" class="main" style="padding:5px;"><b><?php echo TABLE_HEADING_CUSTOMER_NAME ?></b></td>
                <td width="10%" class="main" align="right" style="padding:5px;"><b><?php echo TABLE_HEADING_ORDER ?></b></td>
                <td width="15%" class="main" align="right" style="padding:5px;"><b><?php echo TABLE_HEADING_AMOUNT ?></b></td>
                <td width="20%" class="main" align="right" style="padding:5px;"><b><?php echo TEXT_COUPON_AMOUNT ?></b></td>
            ...
            <?php if (tep_db_num_rows($customer_query) > 0) {
                while ($customer_result = tep_db_fetch_array($customer_query)) {
                    $coupon_amount = ($customer_result['coupon_value'] !== null) ? $currencies->format($customer_result['coupon_value']) : '';

                    echo '<tr class="opencontent" style="padding:5px; cursor:pointer" onclick="location.href=\'' . tep_href_link('edit_orders.php?action=edit&oID=' . $customer_result['orders_id']) . '&action=edit&return=rdc\'">
                            <td class="main" width="25%">' . $customer_result['customers_name'] . '</td>
                            <td class="main" align="right" width="15%">' . $customer_result['orders_id'] . '</td>
                            <td class="main" align="right" width="15%">' . $currencies->format($customer_result['value']) . '</td>
                            <td class="main" align="right" width="15%">' . $coupon_amount . '</td>
                            <td class="main" align="center" width="20%">' . format_date($customer_result['date_purchased']) . '</td>
                          </tr>';
                }
            } else {
                echo '<tr class="opencontent" ><td align="center" class="main" colspan=3 style="padding:5px;">' . TEXT_NO_CUSTOMERS . '</td></tr>';
            }
            echo '</table></div><br>';
		...
		function doMail()
		{
			...
			$uses_per_coupon=0; 
			$customers_use_coupon=0; 
			$coupon_redeem_total=0;
			$uses_per_order=0;
	...
	function doMailPreview()
	{
		...
		/* Re-Post all POST'ed variables */
			reset($FPOST);
			//while (list($key, $value) = each($FPOST)) // PHP 4.3.10
...
<?php
		...
		function doInfo($cid=0)
		{
			...
			$coupon_query=tep_db_query("select c.uses_per_coupon,c.coupon_active,cd.coupon_name,cd.coupon_description,c.coupon_amount,c.coupon_code from " . TABLE_COUPONS . " c , " . TABLE_COUPONS_DESCRIPTION . " cd where c.coupon_id=cd.coupon_id and cd.language_id='" . (int)$FSESSION->languages_id ."' and c.coupon_id='" . tep_db_input($cid) ."'");
			...
			<tr>
				...
				<table border="0" cellpadding="0" cellspacing="0" width="100%">
					<tr>
						...
						<td width="10%" class="main" onClick="javascript:doDisplayAction({'id':'##ID##','get':'##ROW_CLICK_GET##','result':doDisplayResult,'style':'boxRow','type':'##TYPE##','params':'cID=##ID##'});" id="##TYPE####ID##name">##NAME##</td>
						<td width="15%" class="main" onClick="javascript:doDisplayAction({'id':'##ID##','get':'##ROW_CLICK_GET##','result':doDisplayResult,'style':'boxRow','type':'##TYPE##','params':'cID=##ID##'});" id="##TYPE####ID##description">##DESCRIPTION##</td>
              ...
              <tr>
                 ...
                 <?php 
				 	...
				 	//if($uses_per_coupon > $customers_use_coupon){
						...
						// } else 
						// {
						//echo tep_image(tep_output_string(DIR_WS_LANGUAGES . $FSESSION->language . ''), IMAGE_SEND_EMAIL,'','', ' style="cursor:pointer" onClick="javascript:return doUpdateAction({\'id\':\''.$id.'\',\'type\':\'sdc\',\'style\':\'boxRow\',\'imgUpdate\':false,\'validate\':mail_validate,\'uptForm\':\'mail\',\'get\':\'MailPreview\',\'result\':doDisplayResult,\'message1\':\'' .TEXT_CREATING_PREVIEW . '\'});"');
						
						echo '<a onClick="javascript:return doUpdateAction({\'id\':\''.$id.'\',\'type\':\'sdc\',\'style\':\'boxRow\',\'imgUpdate\':false,\'validate\':mail_validate,\'uptForm\':\'mail\',\'get\':\'MailPreview\',\'result\':doDisplayResult,\'message1\':\'' .TEXT_CREATING_PREVIEW . '\'});">' . tep_image_button('', IMAGE_SEND_EMAIL) . '</a> ';
						//}
					//}
				?>
                </td>
