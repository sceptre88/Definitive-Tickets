...
// Check to ensure this file is included in osConcert!
	...
	if(($FREQUEST->isEXists("POST"))>0){
		...
		{
		//PHP8
			//$FREQUEST->setvalue($key,$key,$FPOST[$key],"POST","POST");
			$FREQUEST->setvalue($key,tep_strip_input($key,$FPOST[$key],"POST"),"POST");
			//$FREQUEST->setvalue($key,'',"POST");
		}		
	}
 ...
 // if ($menu_path=='') $menu_path=$FSESSION->get('mPath');
  ...
  $CONFIG_ACCESS_KEY=substr($CONFIG_ACCESS_KEY,0,-1);
   ...
   function tep_strip_input($key,$data,$mode="GET"){
		global $INPUT_FILTER;
		if ($mode=="POST" && (strpos($key,'_description')!==false || $key=='message_text' || $key=='file_contents')) return $data;
		if (is_array($data)) {
			if (count($data)>0){
				reset($data);
