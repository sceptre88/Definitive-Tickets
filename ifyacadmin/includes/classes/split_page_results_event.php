...
// calculate number of pages needing links
      ...
      if ($num_pages > 1) {
        ...
        if (($current_page_number < $num_pages) && ($num_pages != 1)) {
          $display_links .= '&nbsp;&nbsp;<a href="' . tep_href_link(basename($PHP_SELF), $parameters . $page_name . '=' . ((int)$current_page_number + 1), 'NONSSL') . '" class="splitPageLink">' . PREVNEXT_BUTTON_NEXT . '</a>';
...
// calculate number of pages needing links
    ...
    function display_count($query_numrows, $max_rows_per_page, $current_page_number, $text_output) {
      $to_num = ((int)$max_rows_per_page * (int)$current_page_number);
      if ($to_num > $query_numrows) $to_num = (int)$query_numrows;
      $from_num = ((int)$max_rows_per_page * ((int)$current_page_number - 1));
      if ($to_num == 0) {
        $from_num = 0;
      } else {
        $from_num++;
