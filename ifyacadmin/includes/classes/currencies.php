    ...
    if ($calculate_currency_value == true) 
	  {
	    ...
	    $format_string = $this->currencies[$currency_type]['symbol_left'] . number_format(floatval($number) * floatval($rate), $this->currencies[$currency_type]['decimal_places'], $this->currencies[$currency_type]['decimal_point'], $this->currencies[$currency_type]['thousands_point']) . ' '.$this->currencies[$currency_type]['symbol_right'];
	 ...
	 //
	  if(DEFAULT_CURRENCY == 'GBP')
	  {
		...
		if(($post_action=='pdf' || $post_action=='excel' || $post_action=='excel2') && (strpos(",".$this->currencies[$currency_type]['symbol_left'],"Â")>0 || strpos(",".$this->currencies[$currency_type]['symbol_left'],"â‚¬")>0 || strpos(",".$this->currencies[$currency_type]['symbol_left'],"?")>0))
		{
	  ...
	  elseif(DEFAULT_CURRENCY == 'EUR'){
	  	if(!$post_action) $post_action=$command;
		if(($post_action=='pdf' || $post_action=='excel' || $post_action=='excel2') && (strpos(",".$this->currencies[$currency_type]['symbol_left'],"Â")>0 || strpos(",".$this->currencies[$currency_type]['symbol_left'],"?")>0 || strpos(",".$this->currencies[$currency_type]['symbol_left'],"â‚¬")>0)){
	  		$format_string=str_replace($this->currencies[$currency_type]['symbol_left'],"€", $format_string);			
	  	}
		if(($post_action=='pdf' || $post_action=='excel' || $post_action=='excel2') && (strpos(",".$this->currencies[$currency_type]['symbol_right'],"Â")>0 || strpos(",".$this->currencies[$currency_type]['symbol_right'],"?")>0 || strpos(",".$this->currencies[$currency_type]['symbol_right'],"â‚¬")>0)){
	  		$format_string=str_replace($this->currencies[$currency_type]['symbol_right'],"€", $format_string);			
	  	}
	  }
      return $format_string;	 
