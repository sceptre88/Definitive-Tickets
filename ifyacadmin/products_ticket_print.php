...
// Now that you've ensured the directory exists, you can proceed with flushing files if needed
if ($_REQUEST['flush'] && $_REQUEST['flush'] == 'Yes') {
	...
	define("TEXT_BUN","Unique Number");
			...
			################################################################################################
				...
				################################################################################################
					...
					define('TICKET_TEMPLATE', '3');
					}	
				$template_id=3;
				// get the template details if not found use default template
				$template_query=tep_db_query("SELECT template_id,template_width,template_height,template_content from " . TABLE_GENERAL_TEMPLATES . " where template_type='TIC3' and template_id='" . $template_id . "'");
	...
	$pdf = new tFPDF(ORIENTATION, "mm", $resolution, true, 'UTF-8', false);
			...
			foreach($cat_array as $key=>$value)
			{
					...
					p.master_quantity, 
					p.products_ordered 
					FROM  products p,  products_to_categories p2c, products_description pd WHERE p2c.categories_id='".$value."' and p.products_id=p2c.products_id and p.products_status=1 and pd.language_id=1 and pd.products_id=p2c.products_id order by p.products_id ");
...
// Assuming you have the $order_result["currency"] containing the currency ID

$price = substr($order_result["products_price"], 0, -2);

$defaultCurrencyCode = 'USD'; // Replace with your default currency code

// Query for the currency symbols based on the default currency code
$symbol_query = tep_db_query("SELECT currencies_id, symbol_left, symbol_right FROM " . TABLE_CURRENCIES . " WHERE code = '" . $defaultCurrencyCode . "'");

if ($symbol = tep_db_fetch_array($symbol_query)) {
    $currencyId = $symbol['currencies_id'];
    $symbol_left = $symbol['symbol_left'];
    $symbol_right = $symbol['symbol_right'];
} else {
    // If the default currency code is not found, you can set a default currency ID
    // or handle it as needed
    $currencyId = 0; // Set a suitable default value
    $symbol_left = ''; // Replace with your default left symbol
    $symbol_right = ''; // Replace with your default right symbol
}

// Now you can use $currencyId, $symbol_left, and $symbol_right in your code


// Log the debug information to a file
//$logFile = 'debug.log';
//$logMessage = "Symbol Left: '$symbol_left', Symbol Right: '$symbol_right'\n";

// Determine the appropriate symbol placement
if ($symbol_left != '') {
    $details[$product_id][TEXT_CP1] = $symbol_left . $price;
} elseif ($symbol_right != '') {
    $details[$product_id][TEXT_CP1] = $price . $symbol_right;
} else {
    // No symbol
    $details[$product_id][TEXT_CP1] = $price;
}
//$logMessage .= "Final Result: '" . $details[$product_id][TEXT_CP1] . "'\n";
...
// Rest of your PDF generation code
				...
				//$tickets[$row]["ticketholder_name"]=$order_result["ticketholder_name"];
			...
			if (file_exists($tInfo->sponsor_logo_image)){
				...
				if (isset($info_shop)){
					...
					{
					$pdf->Image($tInfo->shop_logo_image,($tInfo->bar_image_position=="L"?$pos_left:0),0,0,65); // adjust image here
					}else
					{
			...
			if (BARCODE !="none")
			{
					...
					##############################################################
						...
						//qrcode
							...
							$qr_width=QR_WIDTH;
							$qr_text=$ticket["products_id"] . $ticket["run"];
      ...
      <p>
       ...
       <a href="javascript:history.go(-2)">[Reset]</a>
        ...
        <tr>
                ...
                <td class="dataTableContent" valign="top"><?php echo tep_draw_pull_down_menu('cat_id', tep_get_category_tree('0', '', '0', $category_array), $_REQUEST['cat_print_id'],'style="width:400px;" '); ?></td>
            
        </tr>
        <tr>
            <td>
                <input type="checkbox" name="foldernames" value="Yes">Use foldernames
            </td>
        </tr>
        <tr>
            <td>
                ...
                <tr>
            <td>
                <input type="checkbox" checked name="flush" value="Yes">Delete existing files on the server before creation.
            </td>
        </tr>
         <tr>
            <td>
