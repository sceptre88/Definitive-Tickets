...
//  define(ADMIN_USER,'admin user');
  ...
  //Graeme Tyson, sakwoya@sakwoya.co.uk,  March  2012 for osConcert
	...
	if ($date_begin=="")
	{
		$sql =  "select date_sub('".getServerDate()."', interval 6 month) begin,'".date('Y-m-d')."' as end";
	...
	$sql="SELECT distinct(op.products_model) from " . TABLE_ORDERS_PRODUCTS . " op WHERE products_model !='' order by products_id asc LIMIT 125";
	//echo $sql;
	$sql_result=tep_db_query($sql);
...
###################################################################################################################
	//count tickets sold + family tickets		

			$ticket_sum_sql="SELECT sum(op.products_quantity) as tickets_sold from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where  op.orders_id=o.orders_id and orders_status!='0' and date_format(o.date_purchased,'%Y-%m-%d')>='".tep_db_input($date_begin)."' and date_format(o.date_purchased,'%Y-%m-%d')<='".tep_db_input($date_end)."' ".$where." ".$stats."  and op.events_type != 'F' and op.events_type != 'X' and op.events_type != 'B' and op.products_sku !=6";
			...
			$family_ticket_sum_sql="SELECT sum(op.products_quantity) as family_sold from " . TABLE_ORDERS_PRODUCTS . " op, " . TABLE_ORDERS . " o where  op.orders_id=o.orders_id and orders_status!='0' and date_format(o.date_purchased,'%Y-%m-%d')>='".tep_db_input($date_begin)."' and date_format(o.date_purchased,'%Y-%m-%d')<='".tep_db_input($date_end)."' ".$where." ".$stats."  and op.events_type = 'F' and op.products_sku !=6";
			
			$family_ticket_sum_query=tep_db_query($family_ticket_sum_sql);
			$row3=tep_db_fetch_array($family_ticket_sum_query);	
			//print_r($fticket_sum_sql);
			$plus=$row3['family_sold']*	FAMILY_TICKET_QTY;
				
			$tickets=$row2['tickets_sold']+$plus;	
...
#######################################################################################################################				
		...
		for ($icnt=0;$icnt<sizeof($db_result);$icnt++) {
			...
			while($row = tep_db_fetch_array($sql_result)) {
				...
				$date_purchased=substr($row["date_purchased"],0,-9);
			...
			$comments=$comments_row["comments"];
				...
				// if needed to display summary or not
				if (!$summary_show) 
				{
					//Code change RM 97/8/7-ONE/5/12
					...
					'col4'=>substr($date_id, 0,10),
					'col11'=>$showname,
					'col2'=>$i,
					'col9'=>$ticket_id,
					'col10'=>$ticket_printed,
					'col3'=>$row['billing_name'],
					'col20'=>$row['customers_email_address'],
					'col8'=>$row['whitelist_ID'],
					'col7'=>$row['products_quantity'],
					'col5'=>$test,
					'col12'=>$payment,
					'col13'=>$date_purchased,
					'col97'=>$row['customers_id'],//$row['whitelist_ID'],
      ...
      echo "&nbsp;". ucfirst($selected_category) ."&nbsp;";
        switch(strtolower($selected_category)){
            ...
            //cartzone
                ...
                //echo '<br>&nbsp;&nbsp;' . tep_draw_checkbox_field("chk_summary",1,$summary_show,'','onClick="javascript:check(1)"') . SUB_TOTALS;
                break;	
        }
    }	?></td>
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			if ($type=='head'){
				if($report_display_header==strtolower($element['content'])){
					...
					$result.="" . TEXT_SEATS . "\n";
					$result.=TEXT_ID . ''.SEP.'' . SEAT_NAME . ''.SEP.'' . ORDER_ID . ''.SEP.'' . TICKET_ID . ''.SEP.'' . TICKET_PRINTED . ''.SEP.'' . DATE_ID . ''.SEP.'' . SHOW . ''.SEP.'' . TEXT_CLIENT . ''.SEP.'' . TEXT_BILLING_NAME . ''.SEP.'' . TEXT_QTY . ''.SEP.'' . TEXT_STATUS . ''.SEP.'' . TEXT_PAYMENT . ''.SEP.'' . TEXT_PURCHASED . ''.SEP.'' . TEXT_AMOUNT . ''.SEP.'' . TEXT_TAX . "\n";
				} else{
					$report_display='reject';
				}
			} else if (($type==$sub) && ($report_display=='accept')){//
			...
			///////////////////////////////////////////////////////////////////////////////////////////
				...
				//Code change RM 
				//$result.=SEAT . "," . ORDER_ID . "," . TICKET_ID . "," . DATE_ID . "," . TEXT_CLIENT . "," . TEXT_BILLING_NAME . "," . TEXT_QTY . "," . TEXT_STATUS . "," . TEXT_AMOUNT . "\n";
			//} else if (($type=='item') && ($report_display=='accept')){
				//$result.=$element["content"] . "\n";
			} else if (($type=='row') && ($report_display=='accept')){
...
########################################################################################################				
			...
			$result.=$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $value . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col3"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' .  '1'.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' .  $currencies->format($element["col100"]/ sizeof($list_tickets)) . ''.SEP.'' . $element["col98"] . "\n";
			}
			...
			// and process as normal
			$result.=
			$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $element["col9"] . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col3"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' . $element["col7"] . ''.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' . $element["col6"] . ''.SEP.'' . $element["col98"] . "\n";
			}
			}else
			{			

			$result.=$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $element["col9"] . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col3"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' . $element["col7"] . ''.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' . $element["col6"] . ''.SEP.'' . $element["col98"] . "\n";
			}
...
########################################################################################################				
			...
			$result.=$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $value . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col97"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' .  '1'.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' .  $currencies->format($element["col100"]/ sizeof($list_tickets)) . ''.SEP.'' . $element["col98"] . "\n";
			}
			...
			// and process as normal
			$result.=
			$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $element["col9"] . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col97"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' . $element["col7"] . ''.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' . $element["col6"] . ''.SEP.'' . $element["col98"] . "\n";
			}
			}else
			{			

			$result.=$element["col2"] . ''.SEP.'' . utf8_decode($element["col"]) . ''.SEP.'' . utf8_decode($element["col1"]) . ''.SEP.'' . $element["col9"] . ''.SEP.'' . $element["col10"] . ''.SEP.'' . $element["col4"] . ''.SEP.'' . utf8_decode($element["col11"]) . ''.SEP.'' . utf8_decode($element["col97"]) . ''.SEP.'' . utf8_decode($element["col8"]) . ''.SEP.'' . $element["col7"] . ''.SEP.'' . utf8_decode($element["col5"]) . ''.SEP.'' . utf8_decode($element["col12"]) . ''.SEP.'' . $element["col13"] . ''.SEP.'' . $element["col6"] . ''.SEP.'' . $element["col98"] . "\n";
			}
