...
//if($startdir) $startdir = preg_replace("/^\//", "${1}", $startdir); //remove leading slash
if(substr($startdir, -1, 1)!='/') $startdir = $startdir . '/'; //add trailing slash
$leadon = $startdir;
if($leadon=='.') $leadon = '';
...
if(!empty($_GET['dir'])) {
	...
	$dirok = true;
	$dirnames = preg_split('///', strip_tags($_GET['dir']));
...
if(!empty($_GET['download']) && $forcedownloads) {
...
elseif(!empty($_GET['sort']) && $_GET['sort']=="size") {
	@natcasesort($dirs); 
	@ksort($files, SORT_NUMERIC);
}
else {
...
<div id="container">
  ...
  <?php
 	 $breadcrumbs = preg_split('///', str_replace($startdir, '', $leadon));
  ...
  <div id="listingcontainer">
	...
	if($allowuploads) {
		...
		//$last = strtolower($phpmaxsize{strlen($phpmaxsize)-1}); // PHP 4.2.0
		$last = strtolower(substr($phpmaxsize, -1)); // PHP 4.3.0
		switch($last) {
			case 'g':
				$phpmaxsize *= 1024;
			case 'm':
