...
/*
  ...
  Copyright (c) 2009-2022 osConcert
...
// Set flag that this is a parent file
	...
	while($order_result=tep_db_fetch_array($order_query))
	{
		...
		$customers_language=$order_result["customers_language"];
		$customers_telephone=$order_result["customers_telephone"];
	...
	include(DIR_WS_LANGUAGES . $customers_language_directory . '/products_ticket.php');
		...
		if ($prev_event!=$product_id)
		{
			if (!isset($details[$product_id]))
			{
				...
				$details[$product_id][TEXT_CDT]= $special.' '.$coupon_discount;
				############################################################
			}
	...
	$ticket_height=$template_result["template_width"]*10;
	...
	if ($found_results)
	{
		...
		for ($tcnt=0;$tcnt<count($tickets);$tcnt++)
		{
			...
			$event[TICKET_TEXT_7]=TEXT_7;
				...
				$cat_ticket_image=$got_parent_id['categories_image_5'];
				...
				$parent_ticket_image=$got_parent_cat_id['categories_image_5'];
				...
				$parent_parent_ticket_image=$got_parent_parent_cat_id['categories_image_5'];
				...
				if (isset($info_shop)){
					...
					{
					$pdf->Image($tInfo->shop_logo_image,($tInfo->bar_image_position=="L"?$pos_left:BG_IMAGE_LEFT),BG_IMAGE_TOP,140,140); //L,T,W,H//
					}else
					{
					$pdf->Image($tInfo->shop_logo_image,($tInfo->bar_image_position=="L"?$pos_width-$info_shop[0]:$pos_width-$info_shop[0]-30),BG_IMAGE_TOP,70,BG_IMAGE_HEIGHT);
					}
			}
			###############################################################	
			...
			if (BARCODE !="none")
			{
					...
					if ((BARCODE == "QR") &&($sku !=6))
					{
						...
						$pdf->Image($qr_filename,70-($qr_width/2),QR_TOP_POSITION+20, $qr_width); //file,x,y,width,height
						
			// texto debajo del QR
							
							$pdf->SetFont('DejaVu','B',12); //sets the font for TOP line (20)
							$pdf->SetXY(30,65);
							$pdf->SetTextColor(255 , 255, 255);
							
							//$pdf->MultiCell(80, 5, $heading_name."\n".$heading_venue."\n".$products_name ."\n"."Bs ".substr($products_price,0,-5)."\n".$showtime."\n\n".$event[TEXT_THN], 0, 'C');
							//$pdf->MultiCell(100, 5, $heading_name."\n".$heading_venue."\n".$products_name ."\n"."Bs ".substr($products_price,0,-5)."\n".substr($showtime,0,-8)."\n\n".$ticketholder_name, 0, 'C');
							/*$pdf->Text(QR_LEFT_POSITION+2,$qr_width+1,$heading_name);
							$pdf->Text(QR_LEFT_POSITION+2,$qr_width+5,$heading_venue);
							$pdf->Text(QR_LEFT_POSITION+3,$qr_width+8,$products_name);
							$pdf->Text(QR_LEFT_POSITION+3,$qr_width+11,substr($showtime,0,-8));
							$pdf->Text(QR_LEFT_POSITION+3,$qr_width+14,"Bs ".substr($products_price,0,-5));
							$pdf->Text(QR_LEFT_POSITION+15,$qr_width+20,$order_id);*/
							
							

			//incicio lado izquierdo
	
							//$pdf->Text(3,5,$heading_name);
							//$pdf->SetXY(0, 4);
							//$pdf->MultiCell(22, 3, $heading_name."\n\n".$products_name ."\n"."Bs ".substr($products_price,0,-5)."\n\n".substr($showtime,0,-8) ."\n\nancho".$pos_width."\n\nalto". $pos_height, 0, 'C');
							//$pdf->MultiCell(25, 3, $heading_name."\n\n".$products_name ."\n\n".substr($showtime,0,-8), 0, 'C');
							/*$pdf->Text(3,8,$heading_venue);
							$pdf->Text(3,11,$products_name);
							$pdf->Text(3,14,substr($showtime,0,-8));
							$pdf->Text(3,18,"Bs ".substr($products_price,0,-5));*/
							//$pdf->Text(15,$qr_width+20,$order_id);
							
							//Fin lado izquierdo

			
			//fin texto debajo del QR
			...
			##############################################################
				...
				$pdf->SetFont('dejavusans','BI',16); //sets the font for TOP line (20)
				...
				{
					if ($icnt>=1) 
					$pdf->SetFont('DejaVu','B',12); //size of MIDDLE written text (12)
					$pdf->SetXY(30,65);
					$pdf->SetTextColor(255 , 255, 255);

					//$pdf->Text(30,$pos_top+TEXT_TOP_POSITION,$splt_line[$icnt]); //manipulate the positioning of all the text
					$pdf->Text($ticket_width/2-$pdf->GetStringWidth($splt_line[$icnt])/2,$pos_top+TEXT_TOP_POSITION,$splt_line[$icnt]);
					if($tic_type=="TIC2")
					{
					...
					$tear=TEAR_LINE;
					if ($icnt>=1) 
					//$pdf->Text(12+50,$pos_top,$splt_line[$icnt]);
					//$pdf->Text(12+110,$pos_top,$splt_line[$icnt]);


					$pdf->Text(50+75,$pos_top,$splt_line[$icnt]);
					$pdf->Text(75+150,$pos_top,$splt_line[$icnt]);	

				//$pdf->Text(25+$tear,$pos_top,$splt_line[$icnt]);
					}
				...
				// conditions BOTTOM
				$pdf->SetFont('DejaVu','',BOTTOM_FONT_SIZE);
				...
				for ($icnt=0;$icnt<count($splt_line);$icnt++)
				{
					...
					$pdf->Text($cond_left+25,$temp_top-3,substr($splt_line[$icnt],0,-25));
					$pdf->Text($cond_left+25,$temp_top,substr($splt_line[$icnt],68));
				}
	...
	}else
	{
		...
		$pdf->AddFont('DejaVu','B','DejaVuSansCondensed-Bold.ttf',true);
		$pdf->SetFont('DejaVu','B',15); //13
		$pdf->Text(5,5,'No se encontraron detalles o este producto NO se puede imprimir');
	}
	$events_tickets=sprintf("entrada_digital_%s_%s",$login_id,time());
	...
	//enviamos el PDF por whatssap
	
	//TOKEN QUE NOS DA FACEBOOK
$token = 'EAAMHKgIhAAwBACkzQY5EcwBiB3squMX0cSyZBucw2GwaEKmDDZCXL9zbGpt3MCfePAlg9U93ZCNfE6Q4gQBkibNsDtAUEFoVg71iyGUkVtUJ7TJmZA75L5Wci2Bqn2g9tMmR6ZCkHrZComuCaugedy9DNEQXJq3LESBoTXnZCNJrKfTZBEcMpeQ2yDkSZCmTpwsRjErgcpYjTuQZDZD';
//NUESTRO TELEFONO

$telefono = $customers_telephone;
if(strlen ($telefono)<9)
{
$telelfono = '591'.$telefono; 
}
//URL A DONDE SE MANDARA EL MENSAJE
$url = 'https://graph.facebook.com/v16.0/116828561350812/messages';
...
$mensaje = ''
        ...
        .                '"type": "document",'
        .                '"document": {'
        .                    '"link": "https://tkl.arte.bo/images/tickets/'.$events_tickets.'.pdf",'
        .                    '"filename": "ticket.pdf"'
        .                '}'
        .            '}'
        .        ']'
