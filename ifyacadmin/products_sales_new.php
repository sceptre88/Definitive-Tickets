...
###################################################################################################################
	//count tickets sold + family tickets		
			...
			$ticket_sum_sql = "SELECT sum(op.products_quantity) as tickets_sold 
                   FROM " . TABLE_ORDERS_PRODUCTS . " op 
                   INNER JOIN " . TABLE_ORDERS . " o 
                   ON op.orders_id = o.orders_id 
                   WHERE orders_status != '0' 
                   AND date_format(o.date_purchased, '%Y-%m-%d') >= '" . tep_db_input($date_begin) . "' 
                   AND date_format(o.date_purchased, '%Y-%m-%d') <= '" . tep_db_input($date_end) . "' 
                   " . $where . " 
                   " . $stats . "  
                   AND op.events_type != 'F' 
                   AND op.events_type != 'X' 
                   AND op.events_type != 'B' 
                   AND op.products_sku != 6";

			$ticket_sum_query = tep_db_query($ticket_sum_sql);
			$row2 = tep_db_fetch_array($ticket_sum_query);

			$family_ticket_sum_sql = "SELECT sum(op.products_quantity) as family_sold 
									  FROM " . TABLE_ORDERS_PRODUCTS . " op 
									  INNER JOIN " . TABLE_ORDERS . " o 
									  ON op.orders_id = o.orders_id 
									  WHERE orders_status != '0' 
...
#######################################################################################################################				
	...
	while($row = tep_db_fetch_array($sql_result)) 
	{
		...
		if ($coupon_total>0) 
		{
			...
			$comments_sql = "select	comments,customer_notified from " . TABLE_ORDERS_STATUS_HISTORY . "  where orders_id=".$order_id." AND customer_notified=0 AND user_added='web' ";
		...
		// if needed to display summary or not osconcert
		if (!$summary_show) 
		{
			...
			'col16'=>str_replace(",", "",$customers_street_address) . ',' . str_replace(",", "",$customers_suburb) . ',' . $customers_postcode . ',' . $customers_state .',' . $customers_country,
			...
			'col98'=>$comments,//$currencies->format($row['products_price']*$row1["products_tax"] / 100),
			...
			{ ?>
                       ...
                       #1 add the field name to the SQL (line 189)
						...
						<td class='dataTableHeadingTitleContent' width="100" ><?php echo TEXT_BILLING_NAME;?></td>
						 <!--osconcert<td class='dataTableHeadingTitleContent' width="50"><?php //echo TEXT_EMAIL;?></td>-->
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) {
			...
			if ($type=='head')
			{
				if($report_display_header==strtolower($element['content'])){
					...
					$result.="" . TEXT_SEATS . "\n";
					$result.=TEXT_ID . "," . SEAT_NAME . "," . ORDER_ID . "," . TICKET_ID . "," . TICKET_PRINTED . "," . DATE_ID . "," . SHOW . "," . TEXT_CLIENT . "," . TEXT_BILLING_NAME . "," . TEXT_QTY . "," . TEXT_STATUS . "," . TEXT_PAYMENT . "," . TEXT_PURCHASED . "," . TEXT_AMOUNT . "," . TEXT_EMAIL . "," . TEXT_COMMENTS . "\n";
				} else{
					$report_display='reject';
				}
			} else if (($type==$sub) && ($report_display=='accept')){//
...
########################################################################################################				
			...
			// and process as normal
			$result.=
			$element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $element["col9"] . "," . $element["col10"] . "," . $element["col4"] . "," . $element["col11"] . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . $element["col7"] . "," . utf8_decode($element["col5"]) . "," . utf8_decode($element["col12"]) . "," . $element["col13"] . "," . $element["col6"] . "," . $element["col20"] . "," . $element["col98"] . "\n";
			}
			}else
			{			

			$result .= $element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $element["col9"] . "," . $element["col10"] . "," . $element["col4"] . "," . $element["col11"] . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . $element["col7"] . "," . utf8_decode($element["col5"]) . "," . utf8_decode($element["col12"]) . "," . $element["col13"] . "," . $element["col6"] . "," . $element["col20"] . ',"' . str_replace(array("\r", "\n", '"'), array(" ", " ", '""'), $element["col98"]) . '"' . "\n";
}
		...
		for ($icnt=0;$icnt<sizeof($output_array);$icnt++) 
		{
			...
			if ($type=='head')
			{
				if($report_display_header==strtolower($element['content']))
				{
					...
					//$result.=$element['content'] . "\n";
					$result.="" . TEXT_SEATS . "\n";
					$result.=TEXT_ID . "," . SEAT_NAME . "," . ORDER_ID . "," . TICKET_ID . "," . DATE_ID . "," . SHOW . "," . TEXT_CLIENT . "," . TEXT_BILLING_NAME . "," . TEXT_COMPANY . "," . TEXT_STREET_ADDRESS . "," . TEXT_SUBURB . "," . TEXT_POSTCODE . "," . TEXT_STATE . "," . TEXT_COUNTRY . "," . TEXT_TELEPHONE . "\n";
				} else
				{
					$report_display='reject';
				}
			...
			$list_tickets=explode("<br/>", rtrim($element["col9"],'<br/>'));
				if(sizeof($list_tickets)>1)
				{
					...
					{
					$result.=
					$element["col2"] . "," . utf8_decode($element["col"]) . "," . utf8_decode($element["col1"]) . "," . $element["col9"] . "," . $element["col4"] . "," . utf8_decode($element["col11"]) . "," . utf8_decode($element["col3"]) . "," . utf8_decode($element["col8"]) . "," . utf8_decode($element["col15"]) . "," . utf8_decode($element["col16"]) . "," . $element["col22"] . "\n";
					}
