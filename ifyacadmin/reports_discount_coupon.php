...
// Set flag that this is a parent file
 ...
 function sort_by_coupon($start_date,$end_date){
	...
	if ($end_date!='')	$where .=" and date_format(crt.redeem_date,'%Y-%m-%d')<='" . tep_db_input($end_date) . "'";
	 ...
	 $display_coupon_query=tep_db_query("select cd.coupon_id,cd.coupon_name,cd.coupon_description from ".TABLE_COUPONS_DESCRIPTION." cd, ".TABLE_COUPON_REDEEM_TRACK." crt where cd.coupon_id=crt.coupon_id and language_id='$FSESSION->languages_id' ". $where." order by coupon_id");
 ...
 echo '<table>';
  ...
  $coupon_query=tep_db_query("select cd.coupon_id,cd.coupon_name,cd.coupon_description,c.coupon_amount,c.coupon_type,cus.customers_firstname,cus.customers_lastname,crt.order_id,crt.email_redeem_id from " . TABLE_CUSTOMERS." cus, ". TABLE_COUPON_REDEEM_TRACK." crt, ".TABLE_COUPONS." c, " .TABLE_COUPONS_DESCRIPTION." cd where cd.language_id='".(int)$FSESSION->languages_id."' and cd.coupon_id=crt.coupon_id and c.coupon_id=crt.coupon_id and crt.customer_id=cus.customers_id".$where.$orderby_query);
//print_r($_SESSION);

  ?>
  <table width="100%" cellpadding="0" cellspacing="0" border="0">
  <tr>
  	<td class="dataTableHeadingTitleContent" width="15%"><?php echo TEXT_COUPON; ?></td>
	<td class="dataTableHeadingTitleContent" width="15%"><?php echo TEXT_CUSTOMER; ?></td>
	<td class="dataTableHeadingTitleContent" width="15%"><?php echo TEXT_COUPON_DESCRIPTION; ?></td>
...
if ($coupon_array['coupon_type'] == 'P') {
    ...
    if (tep_db_num_rows($value_query) > 0) {
        ...
        // Take the absolute value of the coupon amount
        $coupon_array['coupon_amount'] = abs($result['value']);
    } else {
...
//Oct 2013 end
  $coupon_description=$coupon_array['coupon_description,'];
  ...
  $class=($class=="dataTableRowOdd")?'dataTableRowEven':'dataTableRowOdd';
	...
	$chk_flag=true;
	echo '<tr><td class="dataTableHeadingTitleContent" align="right" colspan="2">'.TEXT_SUB_TOTAL.'&nbsp;</td><td align="right" class="dataTableHeadingTitleContent">'.$currencies->format(tep_get_rounded_amount($coupon_array['coupon_amount'])).'</td></tr>';
	$coupon_amount=0;
	}
  ?>
  ...
  <td class="dataTableContent"><?php echo $coupon_array['coupon_description']; ?></td>
...
$total_class=($class=='dataTableRowOdd')?'dataTableRowEven':'dataTableRowOdd';
  if($sort_flag=='N')
  	echo '<tr height="20" class="'.$total_class.'" valign="middle"><td class="dataTableHeadingTitleContent" align="right" colspan="3">'.TEXT_TOTAL.'&nbsp;</td><td align="right" class="dataTableHeadingTitleContent">'.$currencies->format(tep_get_rounded_amount($coupon_amount)).'</td><td class="dataTableHeadingTitleContent" colspan="2">&nbsp;</td></tr>';
  elseif($sort_flag=='Y')
  	echo '<tr height="20" class="'.$total_class.'" valign="middle"><td class="dataTableHeadingTitleContent" align="right" colspan="3">'.TEXT_SUB_TOTAL.'&nbsp;</td><td align="right" class="dataTableHeadingTitleContent">'.$currencies->format(tep_get_rounded_amount($coupon_amount)).'</td><td class="dataTableHeadingTitleContent" colspan="2">&nbsp;</td></tr>';
  if($sort_flag=='Y')	
  echo '<tr height="20" class="'.$total_class.'" valign="middle"><td class="dataTableHeadingTitleContent" align="right" colspan="3">'.TEXT_GRAND_TOTAL.'&nbsp;</td><td align="right" class="dataTableHeadingTitleContent">'.$currencies->format(tep_get_rounded_amount($grand_total)).'</td><td class="dataTableHeadingTitleContent" colspan="2">&nbsp;</td></tr>';	
  } else 
...
<?php
	...
	$coupon_query=tep_db_query("select cd.coupon_id,cd.coupon_name,cd.coupon_description,c.coupon_type,c.coupon_amount,cus.customers_firstname,cus.customers_lastname,crt.order_id,crt.email_redeem_id from " . TABLE_CUSTOMERS." cus, ". TABLE_COUPON_REDEEM_TRACK." crt, ".TABLE_COUPONS." c, " .TABLE_COUPONS_DESCRIPTION." cd where cd.language_id='".(int)$FSESSION->languages_id."' and cd.coupon_id=crt.coupon_id and c.coupon_id=crt.coupon_id and crt.customer_id=cus.customers_id".$where.$orderby_query);	$result="";
	
	$result.=sprintf("%s,\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"",TEXT_COUPON,TEXT_CUSTOMER,TEXT_COUPON_DESCRIPTION,TEXT_VALUE,TEXT_ORDER,TEXT_EMAIL);
	$result.="\n";
	...
	if($coupon_array['email_redeem_id']!='0')
		...
		$email_coupon='C';
			// Hack for 100% discount: if coupon type is percentage and amount is 100, set new amount to 0
			if ($coupon_array['coupon_type'] == 'P' && $coupon_array['coupon_amount'] == 100) {
				$new_amount = 0;
				$exclude_from_total = true;
			} else {
				$new_amount = $coupon_array['coupon_amount'];
			...
			$display_coupon_name,
			$coupon_array['customers_firstname'].' '.$coupon_array['customers_lastname'],
			$coupon_array['coupon_description'],
			$new_amount,
			//$coupon_array['coupon_amount'],
			$coupon_array['order_id'],$email_coupon);
			$result.="\n";
			
			$sub_total += $exclude_from_total ? 0 : $coupon_array['coupon_amount'];
			$grand_total += $exclude_from_total ? 0 : $coupon_array['coupon_amount'];
