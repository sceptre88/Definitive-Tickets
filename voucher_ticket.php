...
// 2021 edited to allow single visit for PWA customers
// change all refererences to $_SESSION['customer_id'] to a new 
// variable $products_ticket_customer so that we can also use
// $_SESSION['temp_customer_id'] that is now set in checkout_success
// both these session values applied in the next block of text

	if ($FSESSION->is_registered('customer_id')) 
		{
			$products_ticket_customer = $FSESSION->customer_id;
		}
	elseif ($FSESSION->is_registered('temp_customer_id')) 
		{
		$products_ticket_customer = $FSESSION->temp_customer_id;
		$FSESSION->remove('temp_customer_id');
		}
	else
		{
	...
	$sql = tep_db_query(sprintf("select * from " . TABLE_ORDERS . " where `orders_id` = '%s' and `customers_id` = '%s'",addslashes($_GET['oID']),addslashes($products_ticket_customer)));
	...
	while($order_result=tep_db_fetch_array($order_query))
	{
		...
		if ($prev_event!=$product_id)
		{
			...
			//	$service_fee=0;
				...
				if ($products_tax > 0)
				{
					$shown_price = tep_add_tax($final_price, $products_tax);
					$tax = (($final_price * $products_tax)/100);
				...
				$total_products_price = $shown_price;
				
				if($symbol_right !='')
				{				
				$details[$product_id][TEXT_CP1]=tep_display_tax_value($total_products_price).$symbol_right;
				//$details[$product_id][TEXT_CP3]=(tep_display_tax_value($total_products_price+$service_fee)).$symbol_right;
				}
				elseif($symbol_left !='')
				{	
				$details[$product_id][TEXT_CP1]=$symbol_left.tep_display_tax_value($total_products_price);
				//$details[$product_id][TEXT_CP3]=$symbol_left.(tep_display_tax_value($total_products_price+$service_fee));
				}
	...
	$events_tickets=sprintf("events_tickets_%s_%s",(int)$FREQUEST->getvalue('oID'),time());
