...
if ($HttpOrigin == "https://rogerevans.org" || $HttpOrigin == "https://ifyac.org" || $HttpOrigin == "http://localhost:8080")
{ 
...
define('TEXT_NO_ADMISSION','Ticket for different show: ');
...
/** ENABLE DEBUG MODE **/
$class='';
$message='' . TEXT_SCAN_TICKETS . '<a href="" target="_blank" rel="nofollow" >Barcode Lite</a>';
// Barcode
		...
		if(is_int($part[1]) && is_int($part[2]) && is_int($part[3]))
		{
			...
			//		$JSON = json_decode($return['data']);
				...
				else
				{
					...
					// Already
					$message=sprintf(
						'<h1>' . TEXT_ALREADY_SCANNED . '<span>%s</span></h1>',
