...
defined('_FEXEC') or die();
  class ot_coupon {
	...
	define('MODULE_ORDER_TOTAL_COUPON_STATUS', '');
	define('MODULE_ORDER_TOTAL_COUPON_SORT_ORDER', '');
	define('MODULE_ORDER_TOTAL_COUPON_INC_SHIPPING', '');
	define('MODULE_ORDER_TOTAL_COUPON_INC_TAX', '');
	define('MODULE_ORDER_TOTAL_COUPON_CALC_TAX', '');
	define('MODULE_ORDER_TOTAL_COUPON_TAX_CLASS', '');
	define('MODULE_ORDER_TOTAL_EMAIL_COUPON_TITLE', '');
	define('MAX_MULTI_COUPONS', '');
	define('TEXT_ENTER_COUPON_CODE', '');
	define('TEXT_COUPON_VALIDATE', '');
	define('TEXT_REDEEM_ANOTHER_VOUCHER', '');
	define('FILENAME_CHECKOUT_PAYMENT', '');
	define('ERROR_INVALID_STARTDATE_COUPON', '');
	define('ERROR_INVALID_FINISDATE_COUPON', '');
	define('TABLE_COUPONS_DISCOUNT_EMAIL', '');
	define('ERROR_NO_INVALID_REDEEM_COUPON', '');
	define('TABLE_COUPON_REDEEM_TRACK', '');
	define('ERROR_INVALID_USES_COUPON', '');
	define('ERROR_INVALID_USES_USER_COUPON', '');
	define('ERROR_NO_REDEEM_CODE', '');
	define('ERROR_LOW_ORDER_TOTAL', '');
	define('ERROR_LESSTHAN_COUPON_PRICE', '');
	define('ERROR_INVALID_FREE_SHIPPING_COUPON', '');
 ...
 if($_SESSION['customer_country_id']==999)
 {
	if ($this->deduction > 0) {
	  ...
	  $bo='Discount Applied';
	  //$bo='$this->title . ':' . $this->coupon_code';
  ...
  {
  global $cart; // RBE.DS: Ensure $cart is available in this function
      global $FSESSION, $order, $currencies;
        ...
        $_SESSION['discount'] = 0;
		/* RBE.DS: Replaced session-based coupon tracking with cart object 
		$_SESSION['coupon_codes'] = array();
		*/
	  error_log('CART CLASS: ' . get_class($GLOBALS['cart'])); // Debugging line
	  $cart->reset_coupons(); // RBE.DS
		if($FSESSION->is_registered('email_redeem_code'))
 		{
...
function calculate_credit($amount) {
  global $cart; // RBE.DS: Ensure $cart is available in this function
	...
	$this->coupon_code = '';
	/* RBE.DS: Replaced session-based coupon tracking with cart object */
	foreach($cart->get_coupons() as $key => $value){ // RBE.DS 
    if ($i > 0){$plus = ' + ';}
    $cc_id = $value;
	...
	//=================================
	$coupon_code = is_array($coupon_result['coupon_code']) ? $coupon_result['coupon_code'][0] : $coupon_result['coupon_code']; // RBE.DS
	$coupon_get = tep_db_query("select coupon_amount, coupon_minimum_order, restrict_to_products, restrict_to_categories, coupon_type from " . TABLE_COUPONS ." where coupon_code = '". tep_db_input($coupon_code) . "'"); // RBE.DS
	//=================================	
