...
/*
  ...
  Updated 2022, osConcert for admin/edit_orders.php
*/
...
defined('_FEXEC') or die();
  ...
  class ot_service_fee {
    ...
    function calculate_discount($amount) {
      ...
      $od_amount = 0;
      if ((MODULE_ORDER_TOTAL_DISCOUNT_DISABLE_WITH_COUPON == 'true') && ($_SESSION['discount']>0)) return $od_amount;
	...
	//  $customer_group_id=get_customer_group_id();
	  ...
	  //2022 use order->info for admin edit orders
	 // if (strstr($_SERVER['SCRIPT_NAME'],"edit_orders.php")){
		//$qty_discount = $this->calculate_rate($this->count_order_contents_with_fee());		  
		//}else{	  
...
// flat rate
	/////////STEFAN 2022/23
	// if(($customer_group_id ==5)or($customer_group_id ==8))
	// {
...
function count_cart_contents_with_fee(){
   global $cart;
	   ...
	   if (strstr($_SERVER['SCRIPT_NAME'],"edit_orders.php"))
	 {
		...
		//$key is the product_id
		//$value is an array qty => 
		
		$query=tep_db_query("SELECT product_type from " . TABLE_PRODUCTS . " where products_id=" .(int)$key);
		$result=tep_db_fetch_array($query);
		if ($result["product_type"]!="B") 
		{
		$total_items = $total_items + $cart->get_quantity($key);
		}		
		}
...
//2022 this emulates the above function for use by admin edit_orders
//where there is no cart() so we use the order->info
function count_order_contents_with_fee(){
   global $cart,$order;
        $total_items = 0;
...
*/
  ...
  //admin function added here - not dependent on products_status
    function tep_get_admin_product_path($products_id) {
    $cPath = '';   

    $category_query = tep_db_query("select p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = '" . (int)$products_id . "'  and p.products_id = p2c.products_id ");
