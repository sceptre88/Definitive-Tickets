...
defined('_FEXEC') or die();
	 ...
	 {
		 $start   =  " and c.concert_date_unix >= '" . tep_db_prepare_input($_GET['date_start_unix'])."' ";
		 //$start_text = TEXT_FROM . $_GET['date_start'];
		 $start_text = TEXT_FROM . strftime(OSCONCERT_FORMAT, strtotime($_GET['date_start']));
	 ...
	 {
		 $end   = " and c.concert_date_unix <= '" . tep_db_prepare_input($_GET['date_end_unix']) ."' ";
		 //$end_text = TEXT_TO . $_GET['date_end'];
		 $end_text = TEXT_TO . strftime(OSCONCERT_FORMAT, strtotime($_GET['date_end']));
	 }else
	 {
		 $end = "";
		 $end_text = ""; 
						...
						if(FEATURED_CATEGORY_BUTTONS=='yes')
						{
							...
							<div class="col-lg-3 col-md-4 col-sm-6" style="text-align:center;border: red 0px solid;">
								<div class="featured-item">
									  ...
									  <h2 style="font-size:14px">' . $categories_name . '</h2>
									  <h6>' . $heading_date . '</h6>
