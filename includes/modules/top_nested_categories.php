...
//USE THIS if using 3 levels of categories e.g Plan ID 4 / 3 / 2/ 
?>
<?php
	#################################################################
	...
	if ($num_categories > 0) 
	{
		...
		while ($nested_categories = tep_db_fetch_array($nested_categories_query)) 
		{
			...
			$cplan_id = $nested_categories['plan_id'];
			...
			else
			{
				if(USE_CINE=='yes')
				{
					...
					<h3 style="margin:0">' . $categories_name . '</h3>
					<h4>' . $heading_venue .': <br>' . $heading_date.' '.$heading_time . '</h4>
					</div></div></a></div>
					<h4 style="margin:0;text-align:center;">' . $categories_name . '<br><span class="smallText">' . $categories_name . ' </span><br> '  . $heading_venue .': ' . $heading_time . '</h4>
					</div>';
					}
				}
				else
				{
					...
					// }
						...
						{
						 $heading_date='<h4>Date: '.$heading_date.'</h4>';
						 $heading_time='<h4>Time: '.$heading_time.'</h4>';
						 $heading_venue='<h4>'.$categories_venue.'</h4>';
						 if ($categories_venue_id == 'SOLD') 
							{
							$select=SOLD_OUT;
					...
					$categories_content .= '
					<div id="ga_listing" class="col-lg-12 col-md-12">
