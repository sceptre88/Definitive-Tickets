...
osConcert
Copyright (c) 2022 osConcert.com
...
// class constructor
	function __construct() 
	{
		...
		{
			$image = '<img class="img-fluid img-mod" src="' . HTTP_SERVER . DIR_WS_HTTP_CATALOG . DIR_WS_IMAGES . $image . '">';
		}else
		{
			...
			for($i=0;$i<sizeof($image_array);$i++)
			{
				...
				{
					$image = '<img class="img-fluid img-mod" src="' . HTTP_SERVER . DIR_WS_HTTP_CATALOG . DIR_WS_IMAGES . $image . $image_array[$i] . '">';
					$image_check = false;
				}
			}
			if($image_check)$image = $path;
		}
		define('MODULE_PAYMENT_PAYPAL_API_TEXT_TITLE',  '<span class="modname">' . $name . '</span>&nbsp;&nbsp;' . $image);
		...
		$this->general_error_heading = MODULE_PAYMENT_PAYPAL_API_ERROR_TITLE;
		...
		$this->form_action_url = tep_href_link('checkout_process.php','','SSL');
...
// class methods
    ...
    function confirmation()
    {
        ...
        if (MODULE_PAYMENT_PAYPAL_API_TEST_MODE=='True') 
		{
		$explanation='TEST MODE<br>Paypal Sandbox in operation.(osConcert: Paypal Sandbox is very unreliable, only recommended for Technicians.';}else{$explanation = '';}
	...
	// OK lets addin a new order status for awaiting
	  $check_query2 = tep_db_query("select orders_status_id from " . TABLE_ORDERS_STATUS . " where orders_status_name = 'Awaiting [PayPal]' limit 1");
      ...
      if (tep_db_num_rows($check_query2) < 1) 
	  {
        ...
        foreach ($languages as $lang) 
		{
          tep_db_query("insert into " . TABLE_ORDERS_STATUS . " (orders_status_id, language_id, orders_status_name) values ('" . $status_id2 . "', '" . $lang['id'] . "', 'Awaiting [PayPal]')");
        }
      } else {
	...
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('PayPal Payment Zone', 'MODULE_PAYMENT_PAYPAL_API_ZONE', '1,2,3', 'If a zone is selected, only enable this payment method for that zone.', '6', '42', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
	...
	//awaiting
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('PayPal Set Awaiting Order Status', 'MODULE_PAYMENT_PAYPAL_API_AWAITS_ORDER_STATUS_ID', '".$status_id2."', 'Set the status of orders with incomplete payment  - <b>do not use</b> Delivered! ', '6', '47', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
	//cancelled
    ...
    function keys() 
	{
		...
		'MODULE_PAYMENT_PAYPAL_API_AWAITS_ORDER_STATUS_ID');//
	}
	...
	$url='https://'.$this->ppurl.'/v1/payment-experience/web-profiles';
     $JSONrequest= '{"name": "'.STORE_NAME.time().rand(0,10000).'"
	,"temporary" : true,"presentation": 
	{
	...
	$result = curl_exec($ch); 
	$resultGetExpreienceProfile = json_decode($result,true);//exit (var_dump($resultGetExpreienceProfile));
	
	$check = $resultGetExpreienceProfile['name']; 
	##################################
	# $check may hold a PayPal error
	##################################
		
	if (tep_not_null($check))
	{

	if($check == "VALIDATION_ERROR")
	{
		$check = $resultGetExpreienceProfile['message'].' '. PROBLEM_ERROR .': '.$resultGetExpreienceProfile['details'][0]['issue'].' '.INFIELD_ERROR.': ['.$resultGetExpreienceProfile['details'][0]['field'];
	

	echo "<script>
		window.location = '".tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $check, 'SSL')."';
		</script>'";
	exit('error');
...
###################################
    ...
    if (tep_not_null($order_id)) 
	{
        ...
        tep_db_query("update " . TABLE_ORDERS_PRODUCTS . " set products_quantity = '0', orders_products_status = MODULE_PAYMENT_PAYPAL_API_CANX_ORDER_STATUS_ID  where orders_id = '" . $order_id . "'");
...
//exit();
	...
	##################################
$check = $resultGetApprovalURL['name']; 
	##################################
	# $check may hold a PayPal error
	##################################
	...
	if($check == "VALIDATION_ERROR")
	{
		...
		$check_your_zip = "";
		if ($resultGetApprovalURL['details'][0]['field'] == 'city'){
				$check_your_zip = MODULE_PAYMENT_PAYPAL_API_VAL_ERROR;
		}
		$check = $resultGetApprovalURL['message'].' '. PROBLEM_ERROR .': '.$resultGetApprovalURL['details'][0]['issue'].' '.INFIELD_ERROR.': ['.$resultGetApprovalURL['details'][0]['field'] . '] '.$check_your_zip; 
