...
class stripesca
{
    ...
    function __construct() {
		...
		}else
		{
			...
			for($i=0;$i<sizeof($image_array);$i++)
			{
				if($image_check && $image != "" && file_exists($path . DIR_WS_IMAGES . $image . $image_array[$i])){
					$image = '<img class="img-fluid img-mod" src="' . HTTP_SERVER . DIR_WS_HTTP_CATALOG . DIR_WS_IMAGES . $image . $image_array[$i] . '" height="33">';
					$image_check = false;
				}
			}
		...
		define('MODULE_PAYMENT_STRIPESCA_TEXT_TITLE',  '<span class="modname">' . $name . '</span>&nbsp;&nbsp;' . $image);
...
/* 		//first work out the store url
		...
		if (MODULE_PAYMENT_STRIPESCA_TESTMODE == 'Test') {//Stripe Payments Test Mode
			$confirmation['title'] .= '<h3>' . MODULE_PAYMENT_STRIPESCA_TEXT_TITLE . '</h3>';
			$confirmation['title'] .= '<div class="messageStackError" style="margin:10px">
			' . MODULE_PAYMENT_STRIPESCA_TEST_MODE . '
    ...
    function before_process()
    {
		...
		require_once('./ext/modules/payment/stripesca/init.php');//exit (HTTPS_SERVER.DIR_WS_HTTPS_CATALOG);
		...
		try{
			...
			$session = \Stripe\Checkout\Session::create([
			  ...
			  'success_url' => HTTPS_SERVER.DIR_WS_HTTPS_CATALOG. "stripesca_success.php"."?c_id=".$_SESSION['customer_id'],
			  'cancel_url' =>  HTTPS_SERVER.DIR_WS_HTTPS_CATALOG. "stripesca_failure.php"."?c_id=".$_SESSION['customer_id'],
		...
		//catch exception
				  ...
				  tep_redirect(tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'payment_error=' . $this->code . '&error=' . $e->getMessage, 'SSL'));
				}
	...
	global $insert_id,  $FSESSION, $merge_details, $send_details, $order;
	
	
//metadata
     try{
             $stuff = array();
			 $stuff['metadata'][TEXT_ORDER_NUMBER.' #: '] =   $insert_id . ' ' .$order->customer['email_address'];
		...
		if (sizeof($products_array)>0){
						...
						for ($i=0, $n=sizeof($products_array); $i<$n; $i++) {
							 {
							 $stuff['metadata'][ITEM.($i+1)] = $products_array[$i]['qty'] . ' x ' . $products_array[$i]['seat'] . ': ' . $products_array[$i]['categories_name'] . ' - ' . $products_array[$i]['venue']. ' - ' . $products_array[$i]['time']. ' - ' . $products_array[$i]['date'];
	...
	function repopulate_cart($order_id,$redirect_page=FILENAME_CHECKOUT_PAYMENT,$extras='', $note='')
{
	...
	if ($status_orig != MODULE_PAYMENT_STRIPESCA_CANX_ORDER_STATUS_ID){
		...
		tep_db_query("DELETE FROM " . TABLE_COUPON_REDEEM_TRACK . "  where order_id = '" . $order_id. "'");
