...
// class constructor
	function __construct() {
		...
		$image = "";
		$path = "";
		if(defined('MODULE_PAYMENT_FREE_DISPLAY_NAME')){
			$name = MODULE_PAYMENT_FREE_DISPLAY_NAME;
		  }
      if(defined('MODULE_PAYMENT_FREE_IMAGE')){
        $image = MODULE_PAYMENT_FREE_IMAGE;
        }
		...
		if($image != "" && file_exists($path . DIR_WS_IMAGES . $image)){
			$image = '<img class="img-fluid img-mod" src="' . HTTP_SERVER . DIR_WS_HTTP_CATALOG . DIR_WS_IMAGES . $image . '" width="103" height="33">';
		}else{
			...
			for($i=0;$i<sizeof($image_array);$i++){
				if($image_check && $image != "" && file_exists($path . DIR_WS_IMAGES . $image . $image_array[$i])){
					$image = '<img class="img-fluid img-mod" src="' . HTTP_SERVER . DIR_WS_HTTP_CATALOG . DIR_WS_IMAGES . $image . $image_array[$i] . '" width="103" height="33">';
					$image_check = false;
				}
			}
		...
		define('MODULE_PAYMENT_FREE_TEXT_TITLE', '<span class="modname">' . $name . '</span>&nbsp;&nbsp;' . $image);
...
// class methods
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Payment Zone', 'MODULE_PAYMENT_FREE_ZONE', '1,2,3', 'If a zone is selected, only enable this payment method for that zone.', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
