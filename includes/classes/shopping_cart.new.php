...
error_log("Loading shoppingCart from " . __FILE__);
	...
	function calculate() 
	{
		...
		//{
			foreach(array_keys($this->contents) as $products_id) 
			{
				...
				$qty = $this->contents[$products_id]['qty'];
				// $owd_costs+=tep_owd_costs('quantity',$this->contents[$products_id]['qty']);  //RBE.DS
				// products price
							...
							if (tep_db_num_rows($sale_query)>0)
							{
								...
								switch($sale['sale_deduction_type']){
									case 1:
										...
										break;
									case 2:
										//$discount_text=$sale["choice_text"] . " (" . $currencies->format($original_price) . ' ' .NEW_PRICE.' '. $currencies->format($sale["sale_deduction_value"]) . ")";
										$discount_text=$sale["choice_text"];
										break;
									default:
...
// =======================
// Coupon Management - RBE.DS
// =======================

public function reset_coupons() {
    $this->coupons = array(); // RBE.DS
    unset($_SESSION['cart_coupons']); // RBE.DS
}

public function add_coupon($code, $is_exclusive = false) {
    if (!isset($this->coupons) || !is_array($this->coupons)) {
        $this->coupons = array(); // RBE.DS
    }

    // Count total tickets in cart
    $ticket_count = $this->count_contents(); // Assumes each item = 1 ticket (adjust if needed)
    $coupon_count = count($this->coupons);

    // If applying a new coupon would exceed ticket count, block it
    if (!$is_exclusive && $coupon_count >= $ticket_count) {
        return false; // RBE.DS: Too many coupons
    }

    if ($is_exclusive) {
        // Exclusive coupon overrides all others
        $this->coupons = array($code => array('code' => $code, 'exclusive' => true)); // RBE.DS
    } else {
        // Prevent adding regular coupon if exclusive one is present
        foreach ($this->coupons as $coupon) {
            if (!empty($coupon['exclusive'])) {
                return false; // RBE.DS: Can't mix exclusive with regular
            }
        }

        // Add regular coupon if it doesn't exceed the ticket count
        $this->coupons[$code] = array('code' => $code, 'exclusive' => false); // RBE.DS
    }

    $_SESSION['cart_coupons'] = $this->coupons; // RBE.DS
    return true; // RBE.DS
}

public function remove_coupon($code) {
    if (isset($this->coupons[$code])) {
        unset($this->coupons[$code]); // RBE.DS
        $_SESSION['cart_coupons'] = $this->coupons; // RBE.DS
    }
}

public function get_coupons() {
    return isset($this->coupons) ? $this->coupons : array(); // RBE.DS
}

public function has_exclusive_coupon() {
    if (!isset($this->coupons)) return false; // RBE.DS
    foreach ($this->coupons as $coupon) {
        if (!empty($coupon['exclusive'])) {
            return true; // RBE.DS
        }
...
if (!function_exists('checkout_compare_price')) {
