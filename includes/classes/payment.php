...
// END ADDED FOR CCGV
  ...
  function before_email() {
    ...
    function stockcheck_fails() {
    if (is_array($this->modules)) {
      reset($this->modules);
      //while (list(, $value) = each($this->modules)) {
      foreach($this->modules as $value) {
        $class = substr($value, 0, strrpos($value, '.'));
        if ($GLOBALS[$class]->enabled && method_exists($GLOBALS[$class], 'stockcheck_fails')) {
          $module_result = $GLOBALS[$class]->stockcheck_fails();
          if ($module_result === true) {
		     return $GLOBALS[$this->selected_module]->stockcheck_fails();
			}else{
			return false;}
        }
      }
