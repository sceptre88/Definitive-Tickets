          ...
          if ($GLOBALS[$class]->enabled) {
			...
			for ($i=0; $i<$size; $i++) 
			{
              $output_string .= '              <tr>' . "\n" .
                                ...
                                '                <td align="right" valign = "top" class="ot">' . $GLOBALS[$class]->output[$i]['text'] . '</td>' . "\n" .
                                '              </tr>';
            }
...
//
    function pre_confirmation_check() {
      ...
      if (MODULE_ORDER_TOTAL_INSTALLED) {
        $total_deductions  = 0;
        reset($this->modules);
        //$order_total = $order->info['total']; //moved to within the loop below
        //while (list(, $value) = each($this->modules)) {
	   //FOREACH
	   foreach($this->modules as $value) {
          $class = substr($value, 0, strrpos($value, '.'));
		  $order_total = $order->info['total'];//moved from 295
