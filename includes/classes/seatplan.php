...
if(!class_exists('seatplan')){
	class seatplan {
		...
		function  tep_renderSeatplanCSS($cPath)
		{
					...
					// Fetch background height once
					$sql = "SELECT bg_height from categories WHERE categories_id=" . $cPath;
					$result = tep_db_query($sql);
					$bg_height = 0; // Initialize with default value
					if ($bg = tep_db_fetch_array($result)) {
			...
			$template_query = tep_db_query("select include_column_left,include_column_right from " . TABLE_TEMPLATE . "  WHERE template_name='" . DEFAULT_TEMPLATE . "'");
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				if($has_column){
				$x=get_percentage(137,$seat['products_x']);
				$y=get_percentage(137,$seat['products_y']);
				$h=26;
				if ($seat['products_w'] >0){
				$w=get_percentage(137,$seat['products_w']);
				}
				$h=26;
				if ($seat['products_h'] >0){
				$f=get_percentage(268,$seat['products_h']);
				$h=get_percentage(137,$seat['products_h']);
				}
				$r=$seat['products_r'];
				$sx=get_percentage(97,$seat['products_sx']);
				$sy=get_percentage(97,$seat['products_sy']);
				}else{
				...
				$trans_css = '';
				if((tep_not_null($seat['products_r']))&&($r!=0)){$trans_css .= "transform:rotate(".$r."deg);";}
				if ($seat['products_sx'] >0){
				if(tep_not_null($seat['products_sx'])){
					$trans_css .= "transform:scale(".$sx.",".$sy.");";
					}	
				}
				$trans_css .= '';
				...
				echo '.btn-ga{font-size:'.$f.'px}' . "\n";
				}
			}	
				if($bg_height>0)
				{
					if($has_column){
					$bg_height=get_percentage(136,$bg_height);
					}else{
					$bg_height=get_percentage(100,$bg_height);	
					}
				echo 'div.seatplan{margin-top:'.$h.'px;height:'.$bg_height.'px;}' . "\n";
				}
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				{
					if($has_column){
					$bg_height=get_percentage(118,$bg_height);
					}else{
					$bg_height=get_percentage(118,$bg_height);	
					}
				echo 'div.seatplan{margin-top:'.$h.'px;height:'.$bg_height.'px;}' . "\n";
			...
			{
				// draggable				
				$x=get_percentage(370,$seat['products_x']);
				$y=get_percentage(370,$seat['products_y']);
				
				if ($seat['products_w'] >0){
				$w=get_percentage(370,$seat['products_w']);
				}
				if ($seat['products_h'] >0){
				...
				if($bg_height>0)
				{
				$bg_height=get_percentage(170,$bg_height);
				echo 'div.seatplan{margin-top:'.$h.'px;height:'.$bg_height.'px;}' . "\n";
				}
				else{
				$bg_height=get_percentage(370,500);//default height set at 500
				echo 'div.seatplan{margin-top:'.$h.'px;height:'.$bg_height.'px;}' . "\n";
				}					
			echo '}' . "\n";
			echo '</style><!--eof style-->
			';
			//End Style
		}
		...
		function  tep_renderSeatplanDesign($cPath)
		{
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				//we use this for default 1100px design space
				...
				switch ($seat['products_status']) 
				{
					...
					//cancel fallthrough for box office:
						if(($_SESSION['customer_country_id']==999)&&(BO_POPOVER=='yes'))
						{
							...
							$order_query = tep_db_query("select customers_id, billing_name from " . TABLE_ORDERS . " where orders_id = '" . (int)$x_order_id . "'");
							$order = tep_db_fetch_array($order_query);		
								
							$titlePrice='order #:'.$x_order_id.'<br />';
							$titlePrice.=$order['billing_name'].'<br />';
						}//end of Box Office
					...
					case 5:
						// if ($_SESSION['draggable']!="yes")
						// {
						// $b='<a class=" btn btn-ga scrollto" style="border:5px solid #000;background-color:'.$seat['color_code']. '" data-bs-toggle="collapse" data-bs-target="#collapse'.$seat['products_url'].'" href="#ga_'.$seat['products_url'].'">'.$seat['products_number'].'</a>';
						// }else{
						$b='<a class="btn btn-ga" style="background-color:'.$seat['color_code'].'" href="#ga_'.$seat['products_url'].'">'.$seat['products_number'].'</a>';
						//}
						if ($seat['product_type']=='L')
						{	
						$di='id="s'.$seat['products_id'].'"'; 
		...
		function tep_renderPricesBar($cPath, $title)
		{
			...
			while ($show = tep_db_fetch_array($result)) 
			{
				...
				///Database Legend
				$html .= '	
				<li class="list-group-item" style="border: none">' . $ticket_type . '
			...
			{
				if((BOX_OFFICE_BLOCKING=='yes')&&(ALLOW_BO_DESIGN=='no'))
				{
				$html .= "</ul>\n";
				$html .= "<button class=\"btn btn-primary btn-sm\"><a class=\"bo-block\" href=\"multi_block.php\">" . TEXT_BO_BLOCKER . "</a></button>\n";
				}
			}
			//$html .= "</ul></div>\n";
			  ...
			  //check for a POST value			  
			   if(isset($_POST['box_office_switch'] ))
			   {
				...
				$html .= tep_draw_form('box_office_refund', 'index.php?'.tep_get_all_get_params($parameters)).
					   tep_draw_hidden_field('box_office_switch','no')."<ul style=\"display: flex;justify-content: center;\"><li class=\"list-group\" style=\"cursor:pointer\" onClick='document.forms[\"box_office_refund\"].submit()'>
					   <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_REFUND_CANCEL . ":</button></li></ul></form>";
				}
				}
				...
				$html .= tep_draw_form('box_office_draggable', 'index.php?'.tep_get_all_get_params($parameters)).
				  ...
				  justify-content: center;\"><li class=\"list-group\" style=\"cursor:pointer\" onClick='document.forms[\"box_office_draggable\"].submit()'>
				  <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_DRAG_CANCEL . "</button></li></ul></form>";
				...
				$html .= tep_draw_form('box_office_reserve', 'index.php?'.tep_get_all_get_params($parameters)).
					   tep_draw_hidden_field('box_office_switch','no')."<ul style=\"display: flex;
				  justify-content: center;\"><li class=\"list-group\" style=\"cursor:pointer\" onClick='document.forms[\"box_office_refund\"].submit()'>
				  <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_RESERVATION_CANCEL . ":</button></li></ul></form>";
				}
				}
				else
				{
					...
					$html .= tep_draw_form('box_office_refund', 'index.php?'.tep_get_all_get_params($parameters)).
					   ...
					   <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_REFUND . ":</button>
					   </li>
					   </ul>
					   </form>";
					}
					...
					$html .= tep_draw_form('box_office_draggable', 'index.php?'.tep_get_all_get_params($parameters)).
					   ...
					   <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_DRAG . "</button>
					   </li>
					   </ul>
					   </form>";
					}
					...
					$html .= tep_draw_form('box_office_reserve', 'index.php?'.tep_get_all_get_params($parameters)).
					   ...
					   <button class=\"btn btn-primary btn-sm\">" . TEXT_LEGEND_RESERVATION . ":</button>
					   </li>
					   </ul>
					   </form>";
					}
		...
		#################################
			...
			// Check if the product has the flag 'is_attributes' set to 'X'
	$check_product_query = tep_db_query("SELECT products_id FROM " . TABLE_PRODUCTS . " WHERE products_id = '" . (int)$products_id . "' AND is_attributes = 'X'");
	...
	{	
		...
		function tep_removeSeat($products_id, $cPath){
			...
			if (stristr($products_id,'-')){
			  ...
			  // $refundArray = explode('-', $products_id);<!--PHP8 -->
			  //[0]= pid. [1]= order_id
...
// we need to add the language here
	...
	function  tep_renderSeatplanRefund($cPath)
		...
		// discount is not shown - only specials price and tax
				...
				switch ($seat['products_status']) 
				{
					...
					case 1:
						// available seats 
						$html .= '<li class="s" id="s' . $seat['products_id'] . '" data-rel="'.$products_original_price.'" title="' . $seat['products_name'] . " - " . $titlePrice . '">' . $A . "</li>";						
						break;
					case 2:
						/* old blank seats */
						$html .= "<li class='b'></li>";
	...
	function  tep_renderSeatplanRefundDesign($cPath)
		{
			global $FSESSION, $currencies, $manufacturers_id,$category_image_3, $category_image_4, $stage_image;
			...
			while ($seat = tep_db_fetch_array($result)) 
			{
				...
				switch ($seat['products_status']) 
				{
					...
					case 5:
						...
						$b='<a class="btn btn-ga" style="background-color:'.$seat['color_code'].'" title="12" href="#ga_'.$seat['products_url'].'">'.$seat['products_number'].'</a>';
						}
						if ($seat['product_type']=='L')
						{	
						$di='id="s'.$seat['products_id'].'"'; 
		...
		/* end: tep_renderSeatplanRefundDesign() */
