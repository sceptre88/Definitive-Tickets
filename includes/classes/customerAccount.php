...
defined('_FEXEC') or die();
	...
	class customerAccount 
	{
		...
		function commonInput($fieldDesc)
		{
			...
			//attempt to hide username completely for pwa
				if((isset($_GET['guest']))&&(PURCHASE_WITHOUT_ACCOUNT=='yes')&&($fieldDesc['uniquename']=='customers_username')){
				echo '' . "\n";
				}else{
			...
			switch($fieldDesc['input_type'])
			{
				...
				case 'O':
						$options=$this->_getOptionArr($fieldDesc['options_values']);
						echo '<div style="border: 0px solid black;width:280px;">';
		...
		//to becontinued
			...
			echo '<div class="col-md-6 text-md-right">' . "\n";	
			echo '<input type="file" name="'. $fieldDesc["uniquename"] . '_file" size="'.$fieldDesc["textbox_size"] .'" accept="image/gif, image/jpeg, image/jpg, image/png"/>';
			echo '</div>' . "\n";
			echo '</div><br>' . "\n";
			...
			echo '<span class="desc">' . $fieldDesc['input_description'] . '</span>';
			if ($img!=""){
				echo '<br>' . tep_image(DIR_WS_IMAGES . "customers/" . $img,'','','','align="absmiddle"');
			}
			echo '<div>';
		}
		...
		function edit__country_state($fieldDesc)
		{
			...
			echo '</div>' . "\n";
			$zones_array = array();
			$zones_query = tep_db_query("select zone_id,zone_name,placement from " . TABLE_ZONES . " where zone_country_id = '" . tep_db_input($country) . "' order by placement DESC, zone_name DESC");
		...
		function edit__customers_referal($fieldDesc)
		{
			...
			{
				$sources_array[] = array('id' => 13, 'text' => 0);
			}
			...
			echo '<div id="refer_other" style="display:none">';
...
if (is_countable($customerAccount) && count($customerAccount)>0){			
