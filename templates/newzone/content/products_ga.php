...
<?php
   ...
   if((int)$FSESSION->customer_id>0 ) $check_restrict_customer=substr($check_restrict_customer,0,-1)." OR (p.restrict_to_customers like '%".(int)$FSESSION->customer_id."%' OR p.restrict_to_groups like '%".tep_db_input($customer_group_id)."%'))";
		...
		p.product_type,
		p.master_quantity 
		from " . TABLE_PRODUCTS . " p join ".TABLE_PRODUCTS_TO_CATEGORIES . " p2c on p2c.products_id=p.products_id join ".TABLE_PRODUCTS_DESCRIPTION . " pd on pd.products_id=p.products_id where p.product_type !='P' and p.product_type !='L' and p.product_type !='Q' and p.products_id >'0' and p2c.categories_id = " . $cPath . " and pd.language_id = " . (int)$FSESSION->languages_id . " ". $check_restrict_customer." order by p.products_sort_order,p.products_id asc");
			...
			if($category_ga['categories_GA']>0)
				{//this is a GA category with master quantity?>
					...
					<?php 
							...
							{
							echo '<strong>' . TEXT_TICKETS_REMAINING . '</strong>';
							}
					?>
					</div>
				<?php 
		...
		{ 
			...
			<div id="ga_<?php echo $listing['products_id']; ?>" class="row p-3 effects">
				<div class="col-md-12">
					...
					<?php 
					echo $pdesc;
					if((ALLOW_PRODUCT_EXPIRY=='yes')&&(SHOW_PRODUCT_EXPIRY=='yes')&&($listing['products_date_available']<'2030-01-01 00:00')){
