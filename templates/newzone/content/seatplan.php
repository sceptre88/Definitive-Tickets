...
?>
	...
	else
	{
		...
		<?php 
		/* how to guide  TOP */
		if((BOX_WIDTH_LEFT=='yes')&&(HOW_TO_GUIDE=='true')){
		require_once(DIR_WS_TEMPLATES.TEMPLATE_NAME.'/content/howto.php');
		}
		?>
		<?php //Fixed Checkout //HIDDEN UNLESS HAS CART CONTENTS
			if ($cart->count_contents() > 0) 
			{
			$sign_up_style="style=\"padding-top:12px;display:true;z-index:100\"";
			}else
			{
		...
		<div id="btnCheckOut" class="fixed-bottom" <?php echo $sign_up_style; ?>>
			<h2><?php 
			echo '<a class="checkout" href="' . tep_href_link('shopping_cart.php') . '">' . sprintf(MODULE_NAVBAR_SHOPPING_CART_HAS_CONTENTS, $cart->count_contents_gt(), $currencies->format($cart->show_total())) . '</a>';
			//echo sprintf(MODULE_NAVBAR_SHOPPING_CART_CONTENTS, $cart->count_contents_gt()); 
			?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<?php echo '<a style="" title="' . IMAGE_BUTTON_CHECKOUT . '" href="' . tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL') . '">' . tep_template_image_button_checkout2('', IMAGE_BUTTON_CHECKOUT) . '</a>'; 
			?></h2>
		</div>
		<?php
	}
	if(HAS_STAGE=='true') //FOR INTEGRATED SEATPLANS
	{
		if($plan_id<5){
		echo"<div id=\"stage_label\">" .  STAGE . "</div>";
		}
	}
    /*  caching */
