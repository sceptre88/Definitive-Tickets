...
defined('_FEXEC') or die();
	...
	function tep_show_static_category($counter) 
	{
		...
		$selected = [];
		if (is_countable($selected[0]) && count($selected[0]) > 0) {
		$selected[0] = $page_display_id ? end($page_display_id) : '';
		}

		$replace_array["CLASS"] = "list-group-item list-group-item-action"; 
		if ($page_display_id && in_array($counter, $selected, true)) {
		  $replace_array["CLASS"] .= " list-group-item-primary active";
		} elseif ($foo[$counter]['parent'] != 0) {
		  $replace_array["CLASS"] .= " list-group-item-action";
		}
