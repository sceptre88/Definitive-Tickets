...
include(DIR_WS_INCLUDES.'javascript/customer_account.js');
	...
	if ((PURCHASE_WITHOUT_ACCOUNT =='yes') && (isset($_GET['guest']) || isset($_POST['guest']))) 
		{
		  $guest_logon = 'guest=guest';
	   }else{
		$guest_logon = '';
	}

echo tep_draw_form('account', tep_href_link(FILENAME_CREATE_ACCOUNT_NEW, $guest_logon, 'SSL'),'POST',' enctype="multipart/form-data" ') . tep_draw_hidden_field('action', 'process');
