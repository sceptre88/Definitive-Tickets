				...
				if (sizeof($products_array)>0)
				{
					...
					for ($i=0, $n=sizeof($products_array); $i<$n; $i++) 
					{
						if (!in_array($products_array[$i]['id'], $products_displayed)) 
						{
							...
							$cPathNow=$products_array[$i]['events_id'];
							$url=tep_href_link(FILENAME_DEFAULT, 'cPath='.$cPathNow.'');
						
							echo 
							$qty_times . '<b><a href="' . $url . '">' . 
							$products_array[$i]['categories_name'] . '</b>  ' . 
							$products_array[$i]['concert_date'] . '  ' .
							$products_array[$i]['concert_time'] . '  ' .							
							$products_array[$i]['seat'] . '</a><br>' ;
