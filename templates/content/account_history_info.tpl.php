...
<div class="container">
	<div class="card mb-4 box-shadow">
		...
		<?php
		//PHP8 if (sizeof($order->info['tax_groups']) > 1) {
		...
		//PHP8 for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) 
		...
		//PHP8 for ($i = 0, $n = sizeof($products); $i < $n; $i++) {
		...
		?>
			...
			<?php
			//PHP8 for ($i=0, $n=sizeof($order->totals); $i<$n; $i++) {
		...
		(DISPLAY_PDF_DELIVERED_ONLY=='true'))
		{
			...
			<div class="row">
			  <div class="col-lg-7 text-center text-lg-left">
				...
				tep_href_link(FILENAME_ACCOUNT_PDF, 'oID=' . $FREQUEST->getvalue('order_id') , 'SSL')); ?>
				</p>	
			  </div>
			  <div class="col-lg-4 cta-btn-container text-center">
				...
				onclick="window.open('<?php echo tep_href_link(FILENAME_ACCOUNT_PDF, 'oID=' . $FREQUEST->getvalue('order_id') , 'SSL'); ?>')">
  ...
  <!-- start vouchers pdf //-->
	...
	if (E_TICKETS == 'true')
	{
		...
		(DISPLAY_PDF_DELIVERED_ONLY=='true'))
		{
			...
			<div class="row">
			  <div class="col-lg-7 text-center text-lg-left">
				<h3 class="cta-title"><?php echo PDF_VOUCHERS; ?></h3>

				<p class="cta-text"><?php echo tep_image(DIR_WS_IMAGES . 'pdf.gif', PRINT_PDF_VOUCHERS,'','','') . sprintf(PDF_VOUCHER_LINK,
				tep_href_link(FILENAME_VOUCHER_PDF, 'oID=' . $FREQUEST->getvalue('order_id') , 'SSL')); ?>
				</p>	
			  </div>
			  <div class="col-lg-4 cta-btn-container text-center">
				<form>
				<input type="button" class="btn btn-primary btn-lg" value="<?php echo GET_VOUCHERS_HERE; ?>"
				onclick="window.open('<?php echo tep_href_link(FILENAME_VOUCHER_PDF, 'oID=' . $FREQUEST->getvalue('order_id') , 'SSL'); ?>')">
				<br>
				</form>
			  </div>
