...
define('TABLE_HEADING_BILLING_ADDRESS', '');
define('TEXT_SELECTED_BILLING_DESTINATION', '');
define('FILENAME_CHECKOUT_PAYMENT_ADDRESS', '');
define('IMAGE_BUTTON_CHANGE_ADDRESS', '');
define('HIDE_CUSTOMER_BILLING_ADDRESS', '');
define('TEXT_SELECTED_PAYMENT_ADDRESS', '');
define('TABLE_HEADING_PAYMENT_METHOD', '');
define('TEXT_SELECT_PAYMENT_METHOD', '');
define('TITLE_PLEASE_SELECT', '');
define('TEXT_ENTER_PAYMENT_INFORMATION', '');
define('TEXT_NO_PAYMENT_SELECTION', '');
define('TEXT_PAYMENT_BARRED', '');
define('TEXT_PAYMENT_BARRED_REASON', '');
define('TEXT_SEASON_TICKET', '');
define('MODULE_ORDER_TOTAL_GV_CHECKBOX', '');
define('TEXT_REDEEM_YOUR_SEASON_TICKET', '');
define('TEXT_USE_COUPON_INSTEAD', '');
define('TEXT_GIFT_VOUCHER', '');
define('TEXT_REDEEM_YOUR_VOUCHER', '');
define('TEXT_COUPON_CANCEL', '');
define('MODULE_ORDER_TOTAL_BOFR_STATUS', '');
define('MODULE_ORDER_TOTAL_BOFR_TITLE', '');
define('TABLE_HEADING_COMMENTS', '');
define('TABLE_HEADING_COMMENTS_HERE', '');
...
define('CHECKOUT_BAR_CONFIRMATION', '');
	...
	echo tep_draw_form('checkout_payment', tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, '', 'SSL'), 'post','onSubmit="" id = "checkout_payment"');
	...
	{
	?>
		<div><?php echo '<span class="alert alert-info">' . tep_output_string_protected($error['title']) . '</span>'; ?></div><br>
	...
	?>
		...
		?>">
		<tr>
		<td class="main" width="50%" valign="top"><?php echo TEXT_SELECTED_BILLING_DESTINATION; ?><br><br><?php echo '<a href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT_ADDRESS, '', 'SSL') . '"><div>' . tep_template_image_button('', IMAGE_BUTTON_CHANGE_ADDRESS) . '</div></a>'; ?>
		</td>
	...
	<?php
		...
		if (HIDE_CUSTOMER_BILLING_ADDRESS == 'no')
		{
		?>
		<table width="100%" cellpadding="2">
              <tr>
                <td class="main" width="50%" valign="top"><?php echo TEXT_SELECTED_PAYMENT_ADDRESS . '<br><a href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT_ADDRESS, '', 'SSL') . '">' . tep_template_image_button_basic('button_change_address.gif', IMAGE_BUTTON_CHANGE_ADDRESS) . '</a>'; ?></td>
                <td align="right" width="50%" valign="top">
				
				<table cellpadding="2">
                  <tr>
                    <td class="main" align="center" valign="top"><?php echo '<b>' . TITLE_BILLING_ADDRESS . '</b>&nbsp;&nbsp;<br>' . tep_image(DIR_WS_IMAGES . 'arrow_south_east.gif'); ?></td>
                    <td class="main" valign="top"><?php echo tep_address_label($FSESSION->customer_id, $FSESSION->billto, true, ' ', '<br>'); ?></td>
                  </tr>
                </table>
...
<br> 
		...
		<table width="100%" cellpadding="2">
			...
			//PHP8
			if (is_countable($selection) && count($selection) > 1) 
			{
			...
			<?php  
			//PHP8
			if (is_countable($selection) && count($selection) > 0) 
			{
			...
			$flag='B';
			$radio_buttons = 0;
			//<!--PHP8 -->//////////////if (is_countable($selection)){
			//$m = count($selection[0]['fields']);
			$m = count((array)$selection[0]['fields']);
			$cnt = 0;
			$barred=0;
			//PHP8  ??==??
			if (is_countable($selection) && count($selection) > 0) 
...
?>
	...
	?>
		...
		<h4><?php echo TEXT_GIFT_VOUCHER;?></h4> 
			&nbsp;&nbsp;&nbsp;
			<span style="background-color: #FFFF00">Please only use 1 applicable discount code per ticket.</span><br />
												 <ul  style="margin-left:8px">
													 <li>child - for children 12 and under - $13 discount</li>
													 <li>student - for age 13 through college - $4 discount</li>
													 <li>military - for active or veteran - $4 discount</li>
													 <li>senior - for age 65 and up - $4 discount</li>
												 </ul>
			<!-- <span style="background-color: #FFFF00">Please do not combine "group" with other discounts.</span>-->
			</span>
		</td>
		</tr>
		<?php
		...
		<!--BOFR stuff-->
				...
				<table>
					...
					<tr>
						...
						<span> <?php echo ORDER_TOTAL_IS; ?> <?php echo $currencies->format($order->info['total']);?></span>  <!--RBE-->
						</td>
					</tr>
				</table>
				</div>	
...
<?php // RBE.DS: Display applied coupons from the cart ?>
<?php
if (!empty($cart->get_coupons())) { // RBE.DS
    echo '<div class="alert alert-info"><h4>Applied Coupons:</h4><ul class="list-group">'; // RBE.DS
    foreach ($cart->get_coupons() as $id) { // RBE.DS
        $code_query = tep_db_query("SELECT coupon_code FROM " . TABLE_COUPONS . " WHERE coupon_id = '" . (int)$id . "'"); // RBE.DS
        if ($code = tep_db_fetch_array($code_query)) { // RBE.DS
            echo '<li class="list-group-item d-flex justify-content-between align-items-center">'; // RBE.DS
            echo htmlspecialchars($code['coupon_code']); // RBE.DS
            echo ' <a class="btn btn-sm btn-danger" href="' . tep_href_link(FILENAME_CHECKOUT_PAYMENT, 'remove_coupon_id=' . (int)$id, 'SSL') . '">Remove</a>'; // RBE.DS
            echo '</li>'; // RBE.DS
        } // RBE.DS
    } // RBE.DS
    echo '</ul></div>'; // RBE.DS
