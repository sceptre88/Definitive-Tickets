...
function tep_template_image_submit($image, $value = '-AltValue-', $parameters = '') 
{
	global $FSESSION;
		...
		$css_submit = '<div><input  
            data-sitekey="'.GOOGLE_CAPTCHA_PUBLIC_KEY_V3.'" 
            data-callback="onSubmit" 
            data-action="contact_us_submit" type="submit" class="btn btn-primary g-recaptcha" value="' . tep_output_string($value) . '" /></div>';
...
// ############## regular button ########################################
function tep_template_image_button_checkout2($image, $value = '-AltValue-', $parameters = '') 
{
	global $FSESSION;
	$image = '<span class="btn btn-primary2">&nbsp;' . tep_output_string($value) . '&nbsp;</span>';
	return $image;
}
// ############## regular button ########################################
