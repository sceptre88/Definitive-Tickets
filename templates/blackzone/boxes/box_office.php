...
define('BO_ORDER_NUMBER', '');

if ($_SESSION['customer_country_id']==999) 
{
 if (!defined('BO_LOCATE')) 
 {
        define('BO_LOCATE', 'Lokalisieren');
    }	
...
<?php
	...
	{
        $customer_info_query = tep_db_query("select customers_id,delivery_country from " . TABLE_ORDERS . " where orders_id = '" . $_POST['box_office_search'] . "'");
        $customer_infoq      = tep_db_fetch_array($customer_info_query);
       // if ($customer_infoq['customers_id'] != $FSESSION->customer_id) 
			
		if ($customer_infoq['delivery_country'] != 'Box Office')
		{
