  ...
  //  $password_confirmation = tep_db_prepare_input($FREQUEST->postvalue('password_confirmation'));
    ...
    if ($error == false) {
      ...
      if (tep_validate_password($password_current, $check_customer['customers_password'],$check_customer["encryption_style"])) {        
		...
		tep_mail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, TEXT_DATA_REQUEST, EMAIL_TEXT, $check_customer['customers_firstname']. " " .$check_customer['customers_lastname'], $check_customer['customers_email_address']);
		tep_mail($check_customer['customers_firstname']. " " .$check_customer['customers_lastname'],$check_customer['customers_email_address'], TEXT_DATA_REQUEST, EMAIL_TEXT2, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS );
