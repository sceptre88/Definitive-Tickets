...
//set/check the sending email flag
	$sending_status=tep_db_query("select email_sending from email_data where order_id='".$this_order_id."'");
	$sending_result= tep_db_fetch_array($sending_status);
	if($sending_result['orders_status'] == 0){
//email not yet sent

          //(a) set the sending flag
          tep_db_query("update email_data set email_sending = 1 where order_id = " . $this_order_id);
          
					//(1) run through the order products list and compile $ order_is printable
					$order_is_printable = 0;

				// Check if $order->products is not null and is an array
				if (!is_null($order->products) && is_array($order->products)) {
					for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
						$order_is_printable += $order->products[$i]['is_printable'];
					}
				} else {
					// Handle the case when $order->products is null or not an array
					// You can set a default value or take appropriate action here.
					// For example:
					$order_is_printable = 0; // Set a default value
					// or
					// throw new Exception("Invalid or null product array"); // Throw an exception
				}

				// Now, $order_is_printable contains the total of 'is_printable' values
