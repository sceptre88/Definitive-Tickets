...
//PHP8
if (is_countable($id_query) && count($id_query)< 1){
...
repopulate_cart($this_order_id,'','',MODULE_PAYMENT_PAYPAL_API_CANX,$void);
	...
	function repopulate_cart($order_id,$redirect_page=FILENAME_CHECKOUT_PAYMENT,$extras='', $note='',$void='')
{
	...
	//debug email, code moved here out of while... loop
			if (MODULE_PAYMENT_PAYPAL_API_DEBUG == 'True' && tep_not_null(MODULE_PAYMENT_PAYPAL_API_DEBUG_EMAIL)){
	             tep_mail('Debug', MODULE_PAYMENT_PAYPAL_API_DEBUG_EMAIL, 'PayPal cancel', "Order #: ".$order_id. " cancelled by customer", STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);	
			}
