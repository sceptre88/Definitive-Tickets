...
$JSONrequest = '{"payer_id":"'.$_GET['PayerID'].'"}';
	...
	curl_close ($ch);
	//exit (print_r($resultExecutePayment));
	$duplicate = "";
...
if (strtoupper($resultExecutePayment['state']) == 'APPROVED' && (($resultExecutePayment['transactions'][0]['related_resources'][0]['sale']['state'] == 'completed'))){ 
	...
	// change order status
				...
				if ($order_result['orders_status']!= $status){//completed status not found 			
					...
					'customer_notified' => '0',
					'comments' => MODULE_PAYMENT_PAYPAL_API_CUSTOMER_RETURNS);
					...
					}// end of order status change to completed 
...
//if delivered then delete email from table
  if ($delivered == 'yes'){
	  ...
	  tep_db_query("update  " . TABLE_ORDERS . "  set ticket_printed='Y' where orders_id = (" . $this_order_id . ")");
		...
		'date_added' => date('Y-m-d H:i:s',getServerDate(false)), 
		'customer_notified' => '1',
		'comments' => MODULE_PAYMENT_PAYPAL_API_TICKETS);

		tep_db_perform(TABLE_ORDERS_STATUS_HISTORY, $sql_data_array);
  }
  }		
					//}//end completed moved here
	...
	// change order status
				...
				$order_result= tep_db_fetch_array($order_status);
					...
					tep_db_perform(TABLE_ORDERS, $sql_data_array, "update", "orders_id='" . $this_order_id. "'");
					    //update status history
					$sql_data_array = array('orders_id' => $this_order_id,
					'orders_status_id' => MODULE_PAYMENT_PAYPAL_API_AWAITS_ORDER_STATUS_ID,
					'date_added' => date('Y-m-d H:i:s',getServerDate(false)), 
					'customer_notified' => '0',
					'comments' => MODULE_PAYMENT_PAYPAL_API_AWAITING);
