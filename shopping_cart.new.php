		...
		function checkout_compare_price($a, $b)
{
  ...
  function set_coupon($coupon_id) {
    if (!in_array($coupon_id, $this->coupons)) {
      $this->coupons[] = $coupon_id;
    }
    $_SESSION['cart_coupons'] = $this->coupons;
  }

  function remove_coupon($coupon_id) {
    $this->coupons = array_diff($this->coupons, [$coupon_id]);
    $_SESSION['cart_coupons'] = $this->coupons;
  }

  function reset_coupons() {
    $this->coupons = array();
    unset($_SESSION['cart_coupons']);
  }
  ...
  function get_coupons() {
    ...
    function reset_coupons() {
        if (isset($_SESSION['coupon_codes'])) {
            unset($_SESSION['coupon_codes']);
        }
        if (isset($_SESSION['email_redeem_code'])) {
            unset($_SESSION['email_redeem_code']);
        }
    } // RBE.DS: Added reset_coupons method to fix fatal error
  }
