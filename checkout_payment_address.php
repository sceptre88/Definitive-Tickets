...
define('BOX_HEADING_BOX_OFFICE', '');

	
//define('BOX_HEADING_BOX_OFFICE', 'Box Office');	
define('TEXT_SELECTED_PAYMENT_ADDRESS', 'Would you like to create or edit a new billing address for this order?');
...
// process a new billing address
    if (tep_not_null($FREQUEST->postvalue('firstname')) && tep_not_null($FREQUEST->postvalue('lastname'))/* && tep_not_null($FREQUEST->postvalue('street_address'))*/) {
	 ...
	 // $customer_telephone = tep_db_prepare_input($FREQUEST->postvalue('customer_telephone'));
	  ...
	  $customer_phone = tep_db_prepare_input($FREQUEST->postvalue('customer_phone'));
       ...
       // $zone_id = 0;
        ...
        if ($entry_state_has_zones == true) {
          ...
          if (tep_db_num_rows($zone_query) == 1) {
            ...
            $error = false; //osconcert set from true to false to stop dropdown menu error NOv 2010
...
*/
      if ($error == false) {
		...
		if (ACCOUNT_TELEPHONE == 'true') $sql_data_array['entry_customer_phone'] = $customer_phone;
        if (ACCOUNT_STATE == 'true') {
          if ($zone_id > 0) {
