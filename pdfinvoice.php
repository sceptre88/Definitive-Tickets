...
if(findextension(PDF_INVOICE_IMAGE) == 'ai' || findextension(PDF_INVOICE_IMAGE) == 'eps'){
	...
	$pdf->RoundedRect(9, $pos_y+5, 93, 45, 2, 'DF');
	...
	$pdf->Text(14,$pos_y+12, tep_html_entity_decode(ENTRY_SOLD_TO));
	$pdf->AddFont('DejaVu','','DejaVuSansCondensed.ttf',true);
	$pdf->AddFont('DejaVu','B','DejaVuSansCondensed-Bold.ttf',true);
	...
	$pdf->SetY($pos_y+14);
    $pdf->Cell(12);
	$pdf->MultiCell(70, 4, tep_html_entity_decode(tep_address_format($order->customer['format_id'], $order->customer, '', '', "\n")),0,'L');
	
	// Draw Box for Delivery Address
	$pdf->SetDrawColor($border_color[0],$border_color[1],$border_color[2]);
	$pdf->SetLineWidth(0.2);
	$pdf->SetFillColor(255);
	$pdf->RoundedRect(108, $pos_y+5, 93, 45, 2, 'DF');
	...
	$pdf->AddFont('DejaVu','B','DejaVuSansCondensed-Bold.ttf',true);
	$pdf->SetFont('DejaVu','',12);
	$pdf->Text(114,$pos_y+12,tep_html_entity_decode(ENTRY_BILL_TO));
	$pdf->AddFont('DejaVu','','DejaVuSansCondensed.ttf',true);
	$pdf->AddFont('DejaVu','B','DejaVuSansCondensed-Bold.ttf',true);
	$pdf->SetFont('DejaVu','',10); //13
	$pdf->SetX(0);
	...
	$pdf->SetFont(PDF_INV_CORE_FONT,'B',10);
	$pdf->Text(58,$pos_y+13, tep_html_entity_decode(PRINT_INVOICE_DATE));	
	$pdf->SetFont(PDF_INV_CORE_FONT,'',10);
	...
	$pdf->SetFont(PDF_INV_CORE_FONT,'B',7);
...
for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
	...
	$price_ex =$currencies->format(substr($order->products[$i]['final_price'],0,6));
	$price_inc = $currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']));
	$subt_ex = $currencies->format(tep_add_tax($order->products[$i]['final_price'], '') * $order->products[$i]['qty']);
	$subt_inc = $currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']) * $order->products[$i]['qty']);
	}else{
	// $price = $currencies->format($order->products[$i]['price'], true, $order->info['currency'], $order->info['currency_value']);
	 ...
	 $price = $currencies->format(tep_add_tax($order->products[$i]['final_price'], $order->products[$i]['tax']), true, $order->info['currency'], $order->info['currency_value']);
	...
	$pdf->SetFont(PDF_INV_CORE_FONT,'',8);
	$pdf->SetY($Y_Table_Position);
	$pdf->SetX(185);//moves the price box//180 = wider
