...
function send_order_email($insert_id)
{
	...
	$check_language_query = tep_db_query("SELECT directory from languages WHERE languages_id = '" . $customers_language . "'");
	$check_language = tep_db_fetch_array($check_language_query);
	$customers_language_directory=$check_language['directory'];	
	$customers_language_directory='english';
...
//------insert customer choosen option eof ----
		...
		$flag_product=false;
			$flag_product=true;
			//change the value to cover currencies if called from admin then there's no currency session so we need this
			//so that the tax gets added correctly
      //2024 - this next line is zeroing the ticket prices as it makes the final_price into a currencies formatted string?
      //commented out
			...
			$products_ordered .= '	
		<tr height="50">
		<td width="10%" style="vertical-align: text-top;text-align:left">
		'. $dummy_quantity .' x 
		</td>
		<td width="10%" style="text-align:left">
		'. tep_product_email_image($stock_values["products_image_1"],$order->products[$i]['name'],'style="width:50px;height:50px"').'
		</td>
		<td width="20%" style="vertical-align: text-top;text-align:left">
		'. $order->products[$i]['name'] .  '
		' . $products_ordered_attributes . (isset($order->products[$i]['discount_whole_text'])?'
		' . $order->products[$i]['discount_whole_text']:'') . '</td>
		...
		<td width="10%" style="vertical-align: text-top;text-align:left">
		'.$currencies->display_price((int)$order->products[$i]['final_price'], (int)$order->products[$i]['tax'], (int)$order->products[$i]['qty'],false).'
		</td>
		</tr>
		<tr>
...
//end function
### season tickets 
        $order_id= $insert_id;//tep_db_prepare_input($_GET['oID']);
