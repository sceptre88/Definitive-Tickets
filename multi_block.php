...
//$categories_query = tep_db_query("select * from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_status > '0' and c.parent_id > 0 and c.plan_id=3 and c.categories_id = cd.categories_id and cd.language_id = '" . (int)$FSESSION->languages_id . "' order by c.sort_order, cd.categories_id");
	...
	$num_categories = tep_db_num_rows($categories_query);
		if ($num_categories > 0) 
		{
			...
			$vid=$categories['venue_id'];
			
			// if($cPath>300){
			// echo '<link href="'.DIR_WS_TEMPLATES . TEMPLATE_NAME.'/assets/css/seatplan2.css" rel="stylesheet">';
			// }
			...
			//('. $vid .') '. $categories['categories_name'] .' ' . $date . '	
